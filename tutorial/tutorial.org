...
#+begin_src latex
  ...
  matching in the database is case insensitive. In substring mode,
  \ty{taxi} searches for a string ``\verb+a b+'' as ``\verb+%a%b%+,
  where \verb+%+ matches any string. Hence the extra match to the
  Influenza A virus whose name contains ``Philadelphia'' followed by
  ``1''.
#+end_src
...
#+begin_src latex
  ...
  # ID      Parent  Name
    ...
    884952  119211  Influenza A...Philadelphia/6371...
  \end{verbatim}
