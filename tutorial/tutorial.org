...
#+begin_src latex
  ...
  result is the much smaller tree in Figure~\ref{fig:446}C.
#+end_src
#+begin_src sh <<query.sh>>=
...
#+begin_src latex
  ...
  or chromosome. Now the three subspecies of \emph{L. pneumophila},
  \emph{pneumophila}, our target, \emph{fraseri}, and
  \emph{pascullei}~\cite{bre88:leg}, are among the top groups sequenced
  (Table~\ref{tab:gen}). In total, there are 45 genomes at the
  subspecies level or below.
  \begin{table}
    \caption{The taxa of \emph{L. pneumophila} ordered by the number of
      high-quality sequenced genomes.}\label{tab:gen}
    \begin{center}
...
#+end_src
#+begin_src latex
  Apart from the subspecies, Table~\ref{tab:gen} also contiains 138
  genomes associated with the species \emph{L. pneumophila}. They should
  belong to one of the three subspecies, but this classification beyond
  the species level has evidently not been made in these cases. In the
  following we first carry out a ``small'' analysis of genomes assigned
  to one of the three subspecies. This is followed by a ``large''
  analysis that also includes the 138 genomes currently not assigned to
  subspecies.
...
#+begin_src latex
  ...
  91891. The second line states the most recent common ancestor of the
  neighbors \emph{and} the targets, \emph{L. pneumophila}, taxon
  ...
  allowing us to filter for the new targets and print their names and
  taxon IDs.
#+end_src
#+begin_src sh <<query.sh>>=
...
#+begin_src latex
  ...
  next task is to extract from this data the phylogenetic---as opposed
  to taxonomic---targets and neighbors. We do this by calculating a
...
#+begin_src latex
  ...
  that are difficult to read, we reduce the names to just the
  single-character prefix before plotting the tree. Figure~\ref{fig:lpn}
  shows that the target clade, node 16, has a number of taxonomic
  neighbors sprinkled in.
  \begin{figure}
    \begin{center}
...
\]
Node 16 is connected to its parent, the root always labeled 1, via an
edge length 0.025.
#+end_export
#+begin_src sh <<query.sh>>=
  fintac lpn.nwk
...
#+begin_src latex
  ...
  seconds and returns 24 fragments totalling 9.1\,kb marker candidates,
  of which 0.7\,kb are \ty{N}s. We save the potential markers in the
  file \ty{lpn.fasta}.
#+end_src
#+begin_src sh <<query.sh>>=
...
#+begin_src latex
  ...
  Table~\ref{tab:gen} contains 138 genomes associated with the species
  \emph{L. pneumophila}. In other words, there are three times more
  genomes than the 45 we analyzed so far still awaiting analysis. And
  while they haven't been classified among the three subspecies, perhaps
  we can still make use of them. So our plan is to add these 138 genomes
  as ``unknown'' to the analysis in the hope that the distribution of
  known taxonomic targets and neighbors will allow us to also divide the
  unknowns into targets of neighbors, thus making use of all data
  available.
...
#+begin_src latex
  ...
  it, and label its nodes. The tree is saved in \ty{lpn.nwk}
#+end_src
#+begin_src sh <<query.sh>>=
  nj lpn.dist |
      midRoot |
...
#+begin_src latex
  ...
  reassuring. Our target clade in this tree is labeled 25.
#+end_src
#+begin_src sh <<query.sh>>=
  fintac -n "^n" lpn.nwk
#+end_src
...
#+begin_src latex
  ...
  genomes, we expanded the target set from
  30 genomes to 159 genomes and the neighbor set from 15 to 24
  genomes. Markers are found by subtracting the neighbors from one
  target and intersecting the result with all other
  targets~\cite{vie24:mar}. This means that an increase in the number of
  genomes analyzed, irrespective of whether they are targets or
  neighbors, can never lead to an increase in marker material. Instead,
  we usually observe a decrease. Still, in spite of our increase in size
  of the target and neighbor set, we ended up with 6.4\,kb candidate
  markers, plenty for marker construction.
  ...
  silico} the specificity and sensitivity of the pair of primers in
  detecting \emph{L. pneumophila subsp. pneumophila}. Primer
