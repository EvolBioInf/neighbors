...
#+begin_src latex
  ...
  taxonomy with their parent, rank, and the counts of the associated
  genomes. Some of these genome counts are quite large, starting with
  the 6,768 genomes associated with the species \emph{Legionella
  pneumophila}. The distribution of genome counts becomes clearer when
  we sort according to it.
#+end_src
#+begin_src sh <<query.sh>>=
...
#+begin_src latex
  ...
  446      445      species     6768     L. p.
  66976    446      serogroup   777      L. p. serogroup 1
  91891    446      subspecies  432      L. p. subsp. pneumophila
  91892    446      subspecies  23       L. p. subsp. fraseri
  66977    446      serogroup   14       L. p. serogroup 2
  66978    446      serogroup   13       L. p. serogroup 3
  66983    446      serogroup   9        L. p. serogroup 8
  91890    446      subspecies  8        L. p. subsp. pascullei
  ...
  \end{verbatim}
#+end_src
#+begin_src latex
...
#+begin_src latex
  ...
  part of Neighbors, and save the tree in \ty{lpn.nwk}.
#+end_src
#+begin_src sh <<small.sh>>=
  nj lpn.dist |
      midRoot |
...
#+end_src
#+begin_export latex
We list the clusters contained in the tree, as one of them often
corresponds to the desired target clade. With default options,
\ty{clusters} detects two clusters in \ty{lpn.nwk}.
#+end_export
#+begin_src sh <<small.sh>>=
  clusters lpn.nwk
#+end_src
#+begin_export latex
\begin{verbatim}
#Cluster Parent Size C
16       1      30   8.71
2        1      15   5.13
\end{verbatim}
#+end_export
#+begin_export latex
Instead of listing clusters, we can also mark them on the tree.
#+end_export
#+begin_src sh <<small.sh>>=
...
#+begin_src latex
  ...
  \begin{figure}
    ...
    \caption{The phylogeny of 45 genomes of \emph{L. pneumophila}
      highlighting taxonomic targets (\emph{t}), neighbors (\emph{n}),
      and the three subspecies \emph{pneumophila}, \emph{fraseri}, and
      \emph{pascullei}. There are six clusters of minimum size 6, which
      are marked with terminal \ty{c}, 16c, 2c, 8c, 18c, 39c, and
      3c.}\label{fig:lpn}
  \end{figure}
#+end_src
#+begin_src sh <<small.sh>>=
...
#+begin_src latex
  ...
  first of all observe that node 16c contains all targets, so it is
  presumably \emph{subsp. pneumophila}. To annotate the two remaining
  clades rooted on node 2c, we look for a leaf from one of the two
...
#+end_src
#+begin_export latex
You might be wondering why nodes 8 and 3 were not identified as
clusters. By default, the minimum cluster size is 10. If we lower that
to 5, nodes 8 and 3 are included in the list of clusters.
#+end_export
#+begin_src sh <<small.sh>>=
...
#+end_export
#+begin_export latex
However, now 16c and 2c are missing from the list of clusters. This is
because by default \ty{clusters} only prints \emph{terminal} clusters,
that is clusters that don't contain other clusters. If we request
nested clusters, 16c and 2c reappear in the list of clusters.
#+end_export
#+begin_src sh <<small.sh>>=
...
#+begin_src latex
  ...
  19       18     5    5.46
  \end{verbatim}
  We can see in Figure~\ref{fig:lpn} that the target clade, node 16c, has
...
16c from our example tree, and reports that this clade contains all 23
...
\]
The parent of node 16c is node 1, the root and the two are connected
by a long branch, as is characteristic of a cluster
(Figure~\ref{fig:lpn}). However, Sometimes \ty{fintac} identifies a
node that doesn't contain all known targets and isn't a cluster. In a
situation like that it might be helpful to use \ty{climt} to climb
from the node identified by \ty{fintac} to the nearest cluster.
#+end_export
#+begin_src sh <<small.sh>>=
  fintac lpn.nwk
#+end_src
#+begin_src latex
  ...
  16c     23       7          84.44      1       0.0246
  \end{verbatim}
#+end_src
#+begin_src latex
  We begin our search for markers by splitting our genomes into targets,
  i. e. clade 16c, and neighbors, the rest. To separate the targets, we
...
#+begin_src latex
  ...
  it, label its nodes, and mark its clusters. Then we save the tree
  in file \ty{lpn.nwk}.
#+end_src
#+begin_src sh <<large.sh>>=
...
#+begin_src latex
  ...
  reassuring. Our target clade in the large tree is cluster 25c.
#+end_src
#+begin_src sh <<large.sh>>=
  fintac -n "^n" lpn.nwk
#+end_src
