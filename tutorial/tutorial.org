#+begin_src latex
  ...
  targets. These unique target regions serve as the basis for
  constructing diagnostic markers. In this Tutorial I demonstrate the
  application of the seven programs in the Neighbors package. We begin
...
the targets are monophyletic, which is necessary for a successful
\ty{fur} run.
\begin{figure}
  \begin{center}
...
#+begin_src latex
  ...
  \ty{cres} used to quantify the output of \ty{fur} is part of the
  \ty{biobox}.
#+end_src
#+begin_src sh <<query.sh>>=
