#+begin_src latex
  ...
  may take. I made these measurements on my run-of-the-mill laptop with
  16 12th Gen Intel i7 CPUs and 32\,MB memory running WSL under
  Windows11.
...
#+end_src
#+begin_src latex
  We now use \ty{makeNeiDb} to construct the taxonomy database,
  \ty{neidb}. This takes less than a minute. Then we move the database
  to the parent directory and return there.
#+end_src
#+begin_src sh <<db.sh>>=
  makeNeiDb
  mv neidb ../
...
#+begin_src latex
  ...
  taxon-ID, 272624, using \ty{taxi} in substring mode (\ty{-s}). All
  matching in the database is case insensitive.
#+end_src
...
#+begin_src latex
  ...
  associated with the species \emph{Legionella pneumophila}. The range
  of genome counts becomes particularly clear if we sort according to
  it.
#+end_src
#+begin_src sh <<query.sh>>=
...
#+begin_src latex
  ...
  I've edited the output of \ty{neighbors} above and of other programs
  in subsequent sections to fit the printed page. The output begins with
  ...
  of the neighbors \emph{and} the targets, \emph{L. pneumophila} with
  taxon-ID 446. The third line is the header of the subsequent table,
  which consists of four columns, type, taxon-ID, name, and
  genomes. There are three possible types, ``t'' for known target,
  ``tt'' for new target, and ``n'' for neighbor.
...
#+begin_src latex
  ...
  tree we just plotted with \ty{plotTree}. First of all, node 16
  contains all targets, so it is presumably
  \emph{subsp. pneumophila}. To annotate the two remaining clades rooted
  on node 2, we look for a leaf from one of the two remaining
  subspecies, say \emph{fraseri}. However, leaves are labeled by their
  file name without any taxon information, which is included in the
  header lines inside the sequence files. We list both the file name and
  the header line by calling on all input files. Then we filter for
  \emph{fraseri}, which we output together with the corresponding file
  name in the preceding line. We restrict our output to the first hit.
#+end_src
#+begin_src sh <<small.sh>>=
  head -n 1 all/*.fna |
      grep -B 1 fraseri |
...
#+begin_src latex
  ...
  \end{verbatim}
  Now we climb from the \emph{fraseri} accession we just found,
  \ty{GCA\_003003865}, toward the root using \ty{climt} in regex
  mode. This shows that node 8 is on this path, so we hypothesize that
  clade 8 is \emph{subsp. fraseri}.
#+end_src
#+begin_src sh <<small.sh>>=
...
#+end_src
#+begin_src latex
  To test this hypothesis, we check the headers of the accessions in
  clade 8 and find that they all belong to \emph{fraseri}.
#+end_src
#+begin_src sh <<small.sh>>=
  pickle 8 lpn.nwk |
      grep -v '^#' |
...
#+begin_src latex
  ...
  headers of the genomes in this clade, they indeed are all classified
  as \emph{pascullei}.
#+end_src
#+begin_src sh <<small.sh>>=
  pickle 3 lpn.nwk |
