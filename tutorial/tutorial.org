#+begin_src latex
  ...
  In this Tutorial we construct an \ty{sqlite3} database from the NCBI
...
#+end_src
#+begin_src latex
  We can now construct the taxonomy database, \ty{tax.db}, using
  \ty{makeNeiDb}. This takes less than half a minute.
#+end_src
#+begin_src sh <<construct.sh>>=
  makeNeiDb
#+end_src
...
#+end_src
#+begin_src sh <<query.sh>>=
  printf "1376" | neighbors -g tax.db
#+end_src
#+begin_src latex
  \begin{verbatim}
  ...
  target 866775, which was already in Figure~\ref{fig:au1}. We download
  the three target sequences, mark their names with prefix ``t'', and
...
#+begin_src latex
  ...
  could use the programs \ty{land} and \ty{pickle} to pick individual
  clades. \ty{land} labels the nodes in a tree, yielding
