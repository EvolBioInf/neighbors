#+begin_src latex
  ...
  to remember, so the program \ty{taxi} takes you from the name of an
  organism to its taxon-ID. The input to \ty{taxi} is a scientific name
  and the taxonomy database. By default, \ty{taxi} carries out a
...
#+begin_src go <<Set usage, Pr.~\ref{pr:tax}>>=
  u := "taxi <scientific-name> <db>"
  p := "Take user from scientific name to taxon-ID."
  e := "taxi \"homo sapiens\" tax.db"
  clio.Usage(u, p, e)
#+end_src
#+begin_src latex
  We import \ty{clio}.
...
#+end_src
#+begin_src latex
  We import \ty{log}, \ty{strconv}, \ty{ioutil}, and \ty{bytes}.
#+end_src
#+begin_src go <<Testing imports, Pr.~\ref{pr:tax}>>=
  "log"
  "strconv"
