...
#+begin_src latex
  ...
  offset and remote execution. The defaults are substring matching, no
  limit, no offset, and local execution.
#+end_src
...
#+begin_src latex
  ...
  arguments. In every other case we get the remaining arguments and
  check there are two of them, the name of the taxon and the database.
  We store both names. Unless the user opted for exact matching, we
  summarize consecutive white space into single blanks. Then we bracket
  the taxon name and any remaining blanks with wild cards.
#+end_src
...
#+begin_src latex
  ...
  The servers adress is \ty{http://localhost:8080/api/v2/taxa}.
  We specify the call arguments as a map of query parameters.
  Lastly we tell \ty{SendGetRequest} to accept only plain
  text. This results in the same representation of data
  we would get from a local execution.
#+end_src
...
#+begin_src latex
  ...
  the exact option, limit and offset, as well as the
  provided name.
#+end_src
...
#+end_src
#+begin_src latex
  Our query is \emph{Homo sapiens}, \emph{Homo\_\_sapiens} with two
  blanks in the middle, \emph{Haemophilus ducreyi}, and
