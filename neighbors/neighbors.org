...
#+begin_src latex
  ...
  the targets. We check that our \ty{mrcaT} isn't also the root.

  The subtree rooted on the \ty{mrcaT} contains the complete set of
  targets. This complete set of targets might contain new targets not
...
#+begin_src latex
  ...
  it at the end. In between, the result consists either of a list of
  accessions, or of the full report.
#+end_src
...
#+begin_src latex
  ...
  only need to flush the tabwriter at the end, not the stdout.
#+end_src
...
#+begin_src latex
  ...
  in the emptied string slice we already used for the targets, and
  sorting that.
#+end_src
...
genomes string. Then print the four columns in our table, type, taxon
ID, name, and genomes.
#+end_export
...
#+begin_src latex
  ...
  levels. Then we read the taxon-IDs and call \ty{calcTarNei}.
#+end_src
#+begin_src go <<Functions, Pr. \ref{pr:neig}>>=
