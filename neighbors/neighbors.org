#+begin_src latex
  ...
  neighbors. For each taxon in these two sets it also returns the
...
#+end_src
#+begin_src latex
  Apart from the version, we declare only the option to restrict the
...
#+begin_src latex
  ...
  of the most recent common ancestors of the targets and of the whole
