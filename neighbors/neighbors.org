...
#+begin_src latex
  ...
  assemblies that belong to each target. It appears that the list
  returned by the method call can contain hyphens in lieu of true
  accessions. So we clean up the list and store it if it contains at
  least one genuine entry.
#+end_src
...
#+end_src
#+begin_export latex
We remove the hyphens from the list of accessions.
#+end_export
...
#+end_src
#+begin_src latex
  Similarly, if a neighbor has at least one genome accession, we store
  it.
#+end_src
