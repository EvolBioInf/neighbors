...
#+end_src
#+begin_src latex
  We use method calls on the database to look up the accessions of the
  assemblies that belong to each target. It appears that if a target has
  no accession, this is returned as a \ty{-}. So we check for this and
  store the target if it has at least one accession.
#+end_src
