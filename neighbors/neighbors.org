...
#+begin_src go <<neighbors.go>>=
  ...
  func Run() {
	  //<<Run function, Pr. \ref{pr:neig}>>
  }
#+end_src
#+begin_src latex
  \epr In the run function we set the name of \ty{neighbors} and its
  usage, declare the options, parse the options, and respond to
  them. Then we open the database connection and parse the input files.
#+end_src
#+begin_src go <<Run function, Pr. \ref{pr:neig}>>=
...
#+end_src
#+begin_src latex
  Apart from the version, we declare six more options:
  \begin{itemize}
  \item \ty{-g}: restrict output to taxa with one or more genomes
  \item \ty{-l}: just list the genomes and their taxon-IDs
  ...
  \item \ty{-o}: output only targets
  \item \ty{-r}: execute program on a remote server
  \item \ty{-D}: execute program with a specific remote database
  \end{itemize}
#+end_src
...
#+end_src
#+begin_src latex
  We respond to the version option, \ty{-v}, remote execution, \ty{-r},
  the target taxon-IDs, \ty{-t}, and the assembly level, \ty{-L}.
#+end_src
#+begin_src go <<Respond to options, Pr. \ref{pr:neig}>>=
  //<<Respond to \ty{-v}, Pr. \ref{pr:neig}>>
  //<<Respond to \ty{-r}, Pr. \ref{pr:neig}>>
...
#+end_src
#+begin_src latex
  If the switch for remote execution is active, we send a request to the
  \ty{neighbors} instead of executing the program locally.
#+end_src
#+begin_src go <<Respond to \ty{-r}, Pr. \ref{pr:neig}>>=
  if *optR || *optDD != "" {
	  //<<Get program options, Pr. \ref{pr:neig}>>
	  //<<Check for extra arguments, Pr. \ref{pr:neig}>>
	  //<<Set misc arguments, Pr. \ref{pr:neig}>>
	  //<<Send request, Pr. \ref{pr:neig}>>
	  //<<Print response, Pr. \ref{pr:neig}>>
  }
#+end_src
#+begin_src latex
  The program options are anything that is declared as an option in \ty{flag}.
  This means it's all of the call arguments except for the unparsed params.
  We also define the extra argumets, these are all the unparsed params. We need
  to sanitize these arguments, removing \ty{-r} and \ty{-D}.
#+end_src
#+begin_src go <<Get program options, Pr. \ref{pr:neig}>>=
  u := flag.NArg()
  options := []util.Option{
...
#+end_src
#+begin_src latex
  If there are unparsed arguments, they are files we need to send using a post
  request. If there aren't any unparsed arguments we still need to check stdin.
  The server calls the program just like we would via the shell. As files are
  passed with file references - their names - we need to tell the server which
  files it needs to create. Creating these files allows the server use the
  same program call as the local one. \ty{stdin} is a similar matter. However,
  we need to sanitize these filenames before using them
#+end_src
#+begin_src go <<Check for extra arguments, Pr. \ref{pr:neig}>>=
  var files []*os.File
  var stdin *os.File
  filenames := flag.Args()
...
#+end_src
#+begin_src latex
  If no files were passed as extra arguments, there might still be data in
  \ty{stdin}. We check whether there is any by looking at some cryptic bits,
  provided by the file's stats.
#+end_src
#+begin_src go <<Handle stdin, Pr. \ref{pr:neig}>>=
...
#+end_src
#+begin_src latex
  We already checked whether we need to send a get or a post request in the
  last step. So depending on this we call the corresponding function in
  \ty{util}. Sending a get request implies here that we don't have any extra
  arguemts, i.e. files, so we might as well leave that parameter empty.
#+end_src
#+begin_src go <<Send request, Pr. \ref{pr:neig}>>=
  var resp string
  if sendPost {
...
#+end_src
#+begin_src latex
  Lastly we need to print the response.
#+end_src
#+begin_src go <<Print response, Pr. \ref{pr:neig}>>=
  fmt.Print(resp)
  return
#+end_src
#+begin_src latex
...
#+begin_src latex
  ...
  connection, the list option, the genomes option, the tab option, the
  map of requested levels, and the targets only option.
#+end_src
...
#+begin_src latex
  ...
  the target-MRCA the analysis-MRCA, \ty{mrcaA}. Unless the user opted
  for targets-only---if the analysis-MRCA has no nodes other than the
  target(s), we keep moving it up the tree until we've found at least
  one neighbor.
#+end_src
...
#+end_src
#+begin_src latex
  We iterate over the sorted accessions and list them and the taxon-ID,
  together with a marker for \emph{target}, the single character \ty{t}.
#+end_src
...
#+end_src
#+begin_src latex
  We list the neighbors and their taxon-IDs together with their marker,
  \ty{n}.
#+end_src
...
#+begin_src latex
  ...
  the genomes option, the tab option, the map of desired accession
  levels, and the targets only option. Then we read the taxon-IDs and
  calculate the targets and neighbors by calling \ty{calcTarNei}.
#+end_src
#+begin_src go <<Functions, Pr. \ref{pr:neig}>>=
...
#+begin_src latex
  ...
  Figure~\ref{fig:hom} and run each one. Every test is made up of a command
  and an expected result. The results are stored in text files \ty{r1.txt},
  \ty{r2.txt} and so on. Different tests may reuse these files, so let's
  make this command-result binding descriptive using a type.
#+end_src
#+begin_src go <<Testing types, Pr. \ref{pr:neig}>>=
  type Test struct {
	  t *exec.Cmd
	  r int
  }
...
#+end_src
#+begin_src latex
  We now construct all the tests and run them afterwards.
#+end_src
...
#+begin_src latex
  ...
  options. We test these four target-IDs both for local and remote execution.
#+end_src
...
#+begin_src latex
  ...
  \ty{-T}, targets, \ty{-t}, level, \ty{-L}, and targets-only, \ty{-o}.
  All of these are tested locally and remotely as well.
#+end_src
...
#+end_src
#+begin_src latex
  We test the targets-only option, \ty{-o}, by limiting the ordinary
  output and the list output for \ty{tid4.txt} to the targets.
#+end_src
#+begin_src go <<Test \ty{-o}, Pr. \ref{pr:neig}>>=
...
#+begin_src latex
  ...
  which is contained in files \ty{r1.txt}, \ty{r2.txt}, and so on. As we
  are testing both local and remote execution, we expect all the results
  to show up twice. We divide i by two to test for this.
#+end_src
