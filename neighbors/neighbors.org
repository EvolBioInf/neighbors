...
#+end_src
#+begin_src latex
  The result consists either of a tab-separated list of accessions with
  taxa names, a formatted list of accessions or of the full report.
#+end_src
#+begin_src go <<Print result, Pr.~\ref{pr:neig}>>=
  if optLT {
	  //<<List accessions with taxa names, Pr.~\ref{pr:neig}>>
  } else if optL {
...
#+end_src
#+begin_export latex
We list the accessions as tab-separated values. The accessions table
has a header consisting of the sample (target or neighbor), the
accession, and the corresponding taxon name. We first list the
targets, then the neighbors.

#+end_export
#+begin_src go <<List accessions with taxa names, Pr.~\ref{pr:neig}>>=
  //<<List targets with taxa names>>
  //<<List neighbors with taxa names>>
#+end_src
#+begin_export latex
For each target we get its name, the list of genome accessions and
print everything.
#+end_export
#+begin_src go <<List targets with taxa names>>=
  sample := "t"
  for _, target := range targets {
...
#+end_src
#+begin_export latex
Likewise, we list the neighbors.
#+end_export
#+begin_src go <<List neighbors with taxa names>>=
    sample = "n"
    for _, neighbor := range neighbors {
