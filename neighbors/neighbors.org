...
\end{center}
where the elements in square brackets are optional.  We iterate over
the entries. From each entry we extract the name, if present, and the
first accession. Then we print the sample, the name, and the
accession.
#+end_export
#+begin_src go <<Print replicon, Pr.~\ref{pr:neig}>>=
  entries := strings.Split(replicon, ";")
  for _, entry := range entries {
	  //<<Extract name, Pr.~\ref{pr:neig}>>
	  //<<Extract accession, Pr.~\ref{pr:neig}>>
	  fmt.Fprintf(w, "%s\t%s\t%s\n", sample, name, acc)
  }
#+end_src
#+begin_src latex
...
#+end_src
#+begin_export latex
The default name is a hyphen. If we do have a name, we trim any
flanking blanks it might contain and replace any remaining blanks with
underscores.
#+end_export
#+begin_src go <<Extract name, Pr.~\ref{pr:neig}>>=
  name := "-"
...
#+end_src
#+begin_export latex
The default accession is the full entry. If that entry contains a
colon, we split it at the colon. If the right substring obtained by
the split contains a slash, we split it at that and take the left
substring as the accession.
#+end_export
#+begin_src go <<Extract accession, Pr.~\ref{pr:neig}>>=
  acc := entry
  if strings.Index(entry, ":") > -1 {
	  acc = strings.Split(acc, ":")[1]
  }
