...
#+end_src
#+begin_src latex
  We look up the genome accessions that belong to each taxon-ID through
  a method call on the database. The genomes are stored in a map between
  taxon-IDs and string slices.
#+end_src
