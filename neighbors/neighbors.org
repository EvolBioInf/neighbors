...
#+end_src
#+begin_src latex
  If the user gave target taxa, we collect them in a slice of integers.
#+end_src
...
#+end_src
#+begin_export latex
We list the accessions in a table, which is printed using a
\ty{Writer}. So we declare the writer, construct it, and print the header
to it. Then we list the targets and neighbors.
#+end_export
