#+begin_src latex
  ...
  The outline of \ty{makeNeiDb} contains hooks for imports, types, and
  the logic of the run function.  \bpr{makeNeiDb}{pr:db}
#+end_src
#+begin_src go <<makeNeiDb.go>>=
  ...
  func Run() {
	  //<<Run function, Pr. \ref{pr:db}>>
  }
#+end_src
#+begin_src latex
  \epr In the run function, we declare the name of \ty{makeNeiDb} and
  its options, set the usage, parse the options, and construct the
  database.
#+end_src
#+begin_src go <<Run function, Pr. \ref{pr:db}>>=
