...
#+end_src
#+begin_src latex
  Apart from the version, we declare five options:
  \begin{itemize}
...
#+end_src
#+begin_src latex
  If the remote switch is active, we retrieve all the parameters
  and store them in a map. This map represents the query arguments
  for the remote execution request. \ty{-l} decides on the output 
  format. The remote equivalent of this is an accept header. We need
  to define this according the \ty{-l}'s value. After parsing we send
  a request using \ty{SendGetRequest} and print its response.
#+end_src
#+begin_src go <<Respond to \ty{-r}, Pr. \ref{pr:dr}>>=
  if *optR {
    ...
    //<<Decide on accept header, Pr. \ref{pr:dr}>>
    if *optLL != "" {
    ...
    var resp string
    //<<Send request, Pr. \ref{pr:dr}>>
		fmt.Print(resp)
		return
  }
#+end_src
#+begin_src latex
  Using \ty{strconv} requires importing it.
#+end_src
#+begin_src go <<Imports, Pr. \ref{pr:dr}>>=
  "strconv"
#+end_src
#+begin_src latex
  The accept header defines the response's output format.
  A list is represented by the header value \ty{text/plain},
  a graph by \ty{text/vnd.graphviz}.
#+end_src
#+begin_src go <<Decide on accept header, Pr. \ref{pr:dr}>>=
  if *optL {
...
#+end_src
#+begin_src latex
  When sending a request we the \ty{SendGetRequest} function,
  passing along the headers and query parameters we just defined.
  We also specify a url which is: 
  \ty{"http://localhost:8080/api/v2/taxa/{taxon_id}/subtree"}.
  This url requires a taxon id which is stored as a call argument.
  We get them by call \ty{flag.Args()}.
#+end_src
#+begin_src go <<Send request, Pr. \ref{pr:dr}>>=
  args := flag.Args()
...
#+begin_src latex
  ...
  (\ty{-n}), genomes (\ty{-g}), list \ty{-l}, assembly level
  (\ty{-L}), and tree level, \ty{-m}.
#+end_src
