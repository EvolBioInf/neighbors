...
#+end_src
#+begin_export latex
We list the subtree in a table consisting of four columns, taxon-ID,
rank, name, and the number of genomes. If the user asked only for taxa
with genomes, this number has to be at least one. We construct this
table with a \ty{tabwriter} to which we write with the function
\ty{listTree}. We flush the \ty{tabwriter} afterwards.
#+end_export
#+begin_src go <<List subtree, Pr.~\ref{pr:dr}>>=
  w := tabwriter.NewWriter(os.Stdout, 0, 1, 2, ' ', 0)
  fmt.Fprint(w, "# Taxid\tRank\tGenomes\tName\n")
  for _, v := range subtree {
	  r := taxdb.Rank(v)
