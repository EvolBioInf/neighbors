...
#+end_src
#+begin_src latex
  We check whether the current node has one or more genomes associated
  with it. If so, we mark its as having a genome and as having a genome
  in its subtree. Having a genome in the subtree is a property that
  propagates up the tree.
#+end_src
#+begin_src go <<Annotate node, Pr.~\ref{pr:dr}>>=
  if len(taxdb.Accessions(v.ID)) > 0 {
	  hasGenome[v.ID] = true
	  hasGsub[v.ID] = true
  }
...
#+end_src
#+begin_export latex
We get the taxon IDs of the subtree by calling \ty{getIDs}. Then we
list the subtree in a table consisting of three columns, taxon ID, and
the number of genomes. We construct this table with a \ty{tabwriter}
to which we write with the function \ty{listTree}. We flush the
\ty{tabwriter} afterwards.
#+end_export
#+begin_src go <<List subtree, Pr.~\ref{pr:dr}>>=
  var ids []int
  ids = getIds(subtree, ids)
  sort.Ints(ids)
  w := tabwriter.NewWriter(os.Stdout, 0, 1, 2, ' ', 0)
  fmt.Fprint(w, "# Taxid\tRank\tGenomes\n")
  for _, id := range ids {
	  r := taxdb.Rank(id)
	  n := len(taxdb.Accessions(id))
	  fmt.Fprintf(w, "%d\t%s\t%d\n", id, r, n)
  }
  w.Flush()
#+end_src
...
#+end_src
#+begin_export latex
In the function \ty{getIds}, we recursively traverse the subtree and
collect its taxon IDs.
#+end_export
#+begin_src go <<Functions, Pr.~\ref{pr:dr}>>=
  func getIds(v *tax.Node, ids []int) []int {
	  if v == nil { return ids }
	  ids = append(ids, v.ID)
	  ids = getIds(v.Child, ids)
	  ids = getIds(v.Sib, ids)
	  return ids
  }
#+end_src
#+begin_src latex
...
#+begin_src go <<Functions, Pr.~\ref{pr:dr}>>=
  func traverseSubtree(w io.Writer, v *tax.Node,
	  ...
	  traverseSubtree(w, v.Child, hasGenome, hasGsub, optN,
		  optG)
