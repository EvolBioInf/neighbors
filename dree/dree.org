...
rank, name, and the number of genomes. We construct this table with a
\ty{tabwriter} to which we write with the function \ty{listTree}. We
flush the \ty{tabwriter} afterwards.
#+end_export
