...
shown in Figure~\ref{fig:db}, \ty{neidb} consists of four
tables, two core tables, \ty{genome} and \ty{taxon}, and two
auxiliary tables pre-computed from the core tables, \ty{genome\_count}
and \ty{search}.
...
The first auxiliary table, \ty{genome\_count}, stores genome counts
...
listed in table \ty{genome} for that taxon. A recursive count it the
...
Genome counts are contained in table \ty{genome\_count}, while taxon
names are in table \ty{taxon} and . Our search thus requires a join of
these two tables. To make searches fast, we pre-compute the join in
the table \ty{search}, which has attributes taxid, name, and score. We
