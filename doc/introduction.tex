...
\begin{table}
\caption{The 14 programs of Neighbors.}\label{tab:pro}
\begin{center}
\begin{tabular}{rlll}
\hline
...
3 & \ty{clusters} & phylogeny & find clusters\\
4 & \ty{dree} & taxonomy & draw tree\\
5 & \ty{fintac} & phylogeny \& taxonomy & find target clade\\
6 & \ty{land} & phylogeny & label nodes\\
7 & \ty{listNeiDbs} & taxonomy & list remote databases\\
8 & \ty{makeNeiDb} & taxonomy & make neighbors database\\
9 & \ty{mrca} & phylogeny & find most recent common ancestor\\
10 & \ty{neighbors} & taxonomy & find neighbors (and targets)\\
11 & \ty{outliers} & numbers & find outliers\\
12 & \ty{pickle} & phylogeny & pick clades\\
13 & \ty{ranks} & taxonomy & distribute genomes among taxonomic ranks\\
14 & \ty{taxi} & taxonomy & get taxon-ID for taxon name\\\hline
\end{tabular}
\end{center}
\end{table}
