...
Users of \ty{fur} need to know the neighbors of their
...
packages \ty{tdb}, \ty{tax}, and \ty{util}, which are also part of the
\ty{neighbors} module.
...
taxon. The program \ty{ants} lets us walk in the opposite direction,
from a taxon to its ancestors. Finally, we can submit the taxa of
interes to \ty{neighbors}, which finds their closest neighbors and the
...
internal nodes with \ty{lads} and then picking individual clades
with \ty{pickle}. The Tutorial shows how the seven
programs \ty{makeNeiDb}, \ty{taxi}, \ty{dree}, \ty{ants},
\ty{neighbors}, \ty{lads}, and \ty{pickle} can be used to discover
