...
collection of nucleotide sequences, \ty{nt}. The published
...
the program \ty{makeNeiDb}, which is based on the
...
Figure~\ref{fig:db}, \ty{neidb} consists of two tables, \ty{genome}
and \ty{taxon}. Each genome comes from an organism identified by its
...
sequenced for a given taxon, the attribute \ty{taxid} in
...
labeling the internal nodes with \ty{land} and then picking clades
with \ty{pickle}. The Tutorial shows how the seven
programs \ty{makeNeiDb}, \ty{taxi}, \ty{dree}, \ty{ants},
\ty{neighbors}, \ty{land}, and \ty{pickle} can be used
