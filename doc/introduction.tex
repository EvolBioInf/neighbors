...
The Neighbors package consists of the 14 programs listed in
Table~\ref{tab:pro}. Seven of these programs are based on the
taxonomy, five on a phylogeny alone, one, \ty{fintac}, on a phylogeny
and the taxonomy, and one, \ty{outliers}, on numerical data.

\begin{table}
\caption{The 14 programs of Neighbors are based on the taxonomy
(\emph{tax.}), or a phylogeny (\emph{phy.}),
or on numbers (\emph{num.}).}\label{tab:pro}
\begin{center}
\begin{tabular}{rlll}
\hline
\# & Name & Based on & Function\\\hline
1 & \ty{ants} & tax. & list ancestors\\
2 & \ty{climt} & phy. & climb tree\\
3 & \ty{clusters} & phy. & find clusters\\
4 & \ty{dree} & tax. & draw tree\\
5 & \ty{fintac} & phy. \& tax. & find target clade\\
6 & \ty{land} & phy. & label nodes\\
7 & \ty{listNeiDbs} & tax. & list remote databases\\
8 & \ty{makeNeiDb} & tax. & make neighbors database\\
9 & \ty{mrca} & phy. & find most recent common ancestor\\
10 & \ty{neighbors} & tax. & find neighbors (and targets)\\
11 & \ty{outliers} & num. & find outliers\\
12 & \ty{pickle} & phy. & pick clades\\
13 & \ty{ranks} & tax. & distribute genomes among taxonomic ranks\\
14 & \ty{taxi} & tax. & get taxon-ID for taxon name\\\hline
\end{tabular}
\end{center}
\end{table}
...
shown in Figure~\ref{fig:db}, \ty{neidb} consists of five tables, of
which three tables are central, \ty{genome}, \ty{taxon},
and \ty{genome\_count}. In addition, there is a collection of image
URLs in \ty{images}, which are linked to taxa via the
table \ty{tax2ima}.
...
\begin{figure}[ht]
  ...
  \input{db}
  \end{center}    
  \caption{The tables of \ty{neidb}; arrows show foreign key
  constraints, primary keys are labeled \emph{pk}.}\label{fig:db}
\end{figure}
...
URL, and an attribution. Since a taxon might have zero, one or more
images, and an image might depict more than one taxon,
tables \ty{image} and \ty{taxon} have a many to many relationship
expressed as the table \ty{tax2ima}.
...
taxa by the number of genome sequences. However, we'd also like to
give preference to taxa that have a common name over taxa that
don't. To achieve this, we calculate the scores in three steps: The
initial scores are equal to the recursive genome counts. Then we
calculate the maximum of these initial scores. In the third step we
increment the scores of taxa with common names by that maximum.  This
final score is then stored as attribute score in table \ty{taxon}.

Once the database is constructed, we can query it. The tutorial shows
how to do that for \emph{Leginonella pneumophila}, a water-borne
pathogen that can cause pneumonia in humans. The program \ty{taxi}
gives us the taxon-ID for our focal subspecies
of \emph{L. pneumophila}, \emph{subsp. pneumophila}.

It is often useful to place this taxon-ID into context, for which we
have two programs, \ty{dree} and \ty{ants}. Starting from a
...
neighbors retrieved from the taxonomy may differ from the phylogeny
calculated from the target and neighbor genomes to determine the final
list of target and neighbor genomes. In that case we'd like to follow
the phylogeny. So we compute a phylogeny from our target and neighbor
genomes. A program for doing this efficiently
is \ty{phylonium}~\cite{klo20:phy}.

The phylogenies of targets and neighbors may comprise hundreds of taxa
of various taxonomic ranks such as ``species'', ``subspecies'', and
``strain''. The program \ty{ranks} shows the distribution of genomes
among the taxonomic ranks of the descendants of a target taxon. To
directly help analyze large phylogenies, Neighbors contains \ty{land}
for labeling nodes, \ty{pickle} for picking nodes,
\ty{clusters} for finding clusters of closely related
taxa, \ty{mrca} for finding the most recent common ancestor of a set
of nodes, \ty{fintac} for finding the target clade, and \ty{climt} for
climbing the phylogenetic tree up or down.
