#+begin_src latex
  ...
  types, methods, and functions. It also has a hook for a special kind
  of package variable, the templates for SQL queries.  \bpa{tdb}{pa:tdb}
#+end_src
#+begin_src go <<tdb.go>>=
  package tdb
...
#+end_src
#+begin_src latex
  We open the four input tables and defer closing them again.
#+end_src
...
#+begin_src latex
  ...
  number of genomes. Hence we load \ty{genome\_count} last.
#+end_src
...
#+end_src
#+begin_src latex
  We iterate over the genomes and insert them.
#+end_src
...
of these leaves become the ``leaves'' in the next round, until there
are no leaves left. So we need to determine the initial set of leaves,
the leaves in the taxonomy. Then we iterate for as long as we have
...
#+end_src
#+begin_export latex
To analyze a leaf, we find its parent, update the parent's genome
...
counts per level. Notice that for a given taxon the levels occupied by
recursive counts are a superset of the levels occupied by raw
counts. So we iterate over the levels of the recursive count.
#+end_export
...
#+begin_src latex
  ...
  ! matches the taxon name, imposes the limit and offset, and returns
  ! the corresponding taxon-IDs and an error.
...
#+end_src
#+begin_src latex
  We declare a template for the taxids query. We ignore the
  recommendation from the Sqlite documentation that the \ty{limit}
  clause should be applied in conjunction with \ty{order by} as this can
  slow down searches considerably and paging should still work.
#+end_src
#+begin_src go <<Query templates, Pa. \ref{pa:tdb}>>=
...
#+begin_src latex
  ...
  lots, depending on which table the access, \ty{genome},
