...
!\ty{OpenTaxonomyDB} exits with an error message.
...
We import \ty{errors}.
#+end_export
#+begin_src go <<Imports, Pr. \ref{pa:tdb}>>=
  "errors"
#+end_src
#+begin_src latex
...
#+begin_src latex
  ...
  We construct the query for the taxon-ID, execute it, and extract the
...
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
  func (t *TaxonomyDB) AccessionTaxid(acc string) (int, error) {
	  var err error
	  taxid := 0
	  //<<Construct taxid query, Pa. \ref{pa:tdb}>>
...
#+end_src
#+begin_src latex
  We construct the taxid query from its template.
#+end_src
#+begin_src go <<Construct taxid query, Pa. \ref{pa:tdb}>>=
