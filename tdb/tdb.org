...
#+begin_src latex
  ...
  !slice accessions of genome assemblies belonging to that taxon.
...
#+begin_src latex
  \subsection{\ty{Names}}
  ! The method \ty{Names} takes as argument a slice of taxon IDs and
  ! returns the names of the taxa.
#+end_src
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+begin_src latex
  ...
  !taxon's rank. We construct the query, execute it, and extract the
  !name.
#+end_src
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+begin_src latex
  \subsection{\ty{Ranks}}
  !The method \ty{Ranks} takes as argument a slice of taxon IDs and
  !returns the names of the taxa.
#+end_src
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+begin_src latex
  \subsection{\ty{Levels}}
  ! The method \ty{Levels} takes as argument a slice of genome
  ! accessions and returns the assembly levels.
#+end_src
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+begin_src latex
  ...
  ! adjusted slice of genome accessions. The input accessions remain
  ! unchanged.
#+end_src
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+begin_src latex
  ...
  \end{figure}
  The outline of our code for testing \ty{tdb} has hooks for imports and
  the testing logic.
#+end_src
...
#+end_src
#+begin_src latex
  We test the function \ty{NewTaxonomyDB}, and the nine methods
  \ty{Names}, \ty{Ranks}, \ty{Parent}, \ty{Children}, \ty{Subtree},
  \ty{Taxids}, \ty{MRCA}, \ty{Levels}, and \ty{FilterAccessions}. The
  three untested methods \ty{Name}, \ty{Rank}, and \ty{Level} are tested
  via their plural forms.
#+end_src
#+begin_src go <<Testing, Pa. \ref{pa:tdb}>>=
  //<<Test \ty{NewTaxonomyDB}, Pa. \ref{pa:tdb}>>
  //<<Test \ty{Names}, Pa. \ref{pa:tdb}>>
  //<<Test \ty{Ranks}, Pa. \ref{pa:tdb}>>
...
#+end_src
#+begin_src latex
  We look up the names of taxa 9606 and 9605, \emph{Homo sapiens} and
  \emph{Homo}.
#+end_src
#+begin_src go <<Test \ty{Names}, Pa. \ref{pa:tdb}>>=
  tids := []int{9606, 9605}
  gs := taxdb.Names(tids)
...
We import \ty{strings}.
#+end_export
#+begin_src go <<Testing imports, Pa. \ref{pa:tdb}>>=
  "strings"
#+end_src
#+begin_export latex
We look upt the ranks of taxa 9606 and 9605, species and genus.
#+end_export
#+begin_src go <<Test \ty{Ranks}, Pa. \ref{pa:tdb}>>=
  gs = taxdb.Ranks(tids)
...
#+end_src
#+begin_src latex
  The levels of accession GCA\_963575185.1 and
  GCA\_049640585.1 are scaffold and contig.
#+end_src
#+begin_src go <<Test \ty{Levels}, Pa. \ref{pa:tdb}>>=
