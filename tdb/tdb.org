...
#+end_src
#+begin_src latex
  We store the parents and children as integer slices. The children
...
#+end_src
#+begin_src latex
  As long as we haven't found the MRCA yet, we climb to the next set of
  parents and replace the children by the parents.
#+end_src
...
#+end_src
#+begin_src latex
  We iterate over the children. For each child we look up the parent,
  deal with the descendants, and climb to the parent by appending the
...
#+begin_src go <<Climb to parents, Pa. \ref{pa:tdb}>>=
  for _, child := range children {
	  ...
	  //<<Deal with descendants, Pa. \ref{pa:tdb}>>
	  parents = append(parents, parent)
  }
#+end_src
#+begin_export latex
For a given child/parent pair, we increment the parents' descendants
count by that of the child. The only exception to this behavior is
when we have reached the root, in which case the child is equal to its
parent. In that case we have to avoid idly incrementing the root's
descendant count. Then we check whether the number of the parent's
descendants is greater or equal to the number of taxa submitted. If
yes, we've found the MRCA, store it, and break from the iteration over
the children.
#+end_export
#+begin_src go <<Deal with descendants, Pa. \ref{pa:tdb}>>=
  if parent != child {
...
parents and reset the parent slice to empty.
#+end_src
