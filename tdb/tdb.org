...
#+begin_src latex
  ...
  \ty{taxon}, followed by \ty{genome\_count}. Then we return to table
  \ty{taxon} and load the score. We end by loading the two tables dealing
  with images, \ty{image} and \ty{tax2ima}. Since \ty{tax2ima} refers to
  \ty{image}, we start with \ty{image}.
#+end_src
