...
#+begin_src latex
  ...
  cannot open the database, we abort. We also make sure the database is
...
#+begin_src latex
  ...
  \ty{accession}, and \ty{status}. The size is given in bp, but we store
...
#+begin_src latex
  ...
  in Figure~\ref{fig:db}, \ty{taxid}, \ty{accession}, \ty{status}, and
...
#+end_src
#+begin_src latex
  With a handful exceptions, the refseq genomes are a subset of the
...
and return a genome. A genome consists of a accession, a taxon ID, a
status, and a size. As shown in Table~\ref{tab:pg}, the accession is
the first field, the taxon ID the sixth, the status the 12th, and the
genome size the 26. The size is given in bp, which we convert to Mb.
#+end_export
#+begin_src go <<Functions, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_src latex
    To insert the genomes, we iterate over the genbank
    accessions. We extract the data fields, lookup or construct the
    genome for the corresponding accession, and insert the genome.
#+end_src
...
#+begin_src latex
  ...
  \ty{Name}, \ty{Parent}, \ty{Children}, \ty{Subtree},
  and \ty{Taxids}.
#+end_src
