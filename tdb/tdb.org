...
#+begin_src latex
  ...
  them we climb the tree to the root and increment a descendant counter
  for each node. The we climb the last taxon and the first descendant
  counter equal to the number of input taxa is the MRCA.
...
#+end_src
#+begin_export latex
We initialize the descendants counts of each taxon to 1.
#+end_export
...
#+end_src
#+begin_export latex
We have reached the last taxon. It might be the MRCA. If not, we climb
the tree one last time. If we didn't find the MRCA, we message this
via a new error.
#+end_export
