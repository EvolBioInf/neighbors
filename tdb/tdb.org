...
Given this constraint we to test these methods: 
\ty{Taxids}
\ty{CommonTaxids}
\ty{Level}
\ty{NumGenomes}
...
filter names with the like operator. In order to check for injectability
...
#+end_src
#+begin_export latex
Our goal is to escape the filtering done by like and return everything instead. 
We use two test strings one for string enclosed names and one for non enclosed one.
The non enclosed one could be something like '' or 1=1. Here '' catches the like 
operator and allows us to set another condition which 
always evaluates to true.
The enclosed one looks similar and might be something like this: ' or 1=1 or '
where the first apostrophe breaks us out of literal and the last reenters to 
...
injection. Here we could escape the query to create a new return and return something
totally different.
#+end_export
...
In this case we need to return an int, so we again escape the query
and return -1.
#+end_export
...
#+end_src
#+begin_export latex
We expect that \ty{NumGenomes} is injection safe meaning both queries find
a zero as there is neither a taxon with id -1 nor a level with that cryptic
...
#+end_src
#+begin_export latex
We expect that \ty{NumGenomesRec} is injection safe meaning both queries find
a zero as there is neither a taxon with id -1 nor a level with that cryptic
name we defined above.
#+end_export
...
#+end_src
#+begin_export latex
Lastly we check for injections \ty{AccessionTaxid}. We use the same schema as we 
did with the last two checks. We can reuse the injection strings as well.
#+end_export
...
#+end_src
#+begin_export latex
We expect that \ty{AccessionTaxid} is injection safe meaning both queries find
a zero as there is no taxon with the cryptic name defined above.
#+end_export
