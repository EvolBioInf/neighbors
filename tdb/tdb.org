...
#+begin_src latex
  \subsection{\ty{MRCA}}
  The method MRCA returns the most recent common ancestor of a sample of
  taxa. The MRCA is computed by climbing from the given taxa toward the
  root of the taxonomy. While climbing, the number of descendants is
  passed from child to parent. The climb stops when a taxon is reached
  with as many descendants as taxa in the original sample. That taxon is
  the MRCA.  We implement the search for the MRCA using two maps, the
  set of active lines, and the number of descendants per taxon.
#+end_src
#+begin_src go <<Methods, Pa.~\ref{pa:tdb}>>=
  // The method MRCA returns the most recent common ancestor of a slice
  // of taxon IDs.
  func (t *TaxonomyDB) MRCA(taxa []int) int {
	  mrca := 0
	  //<<Construct set of lines, Pa.~\ref{pa:tdb}>>
	  //<<Construct descendants counter, Pa.~\ref{pa:tdb}>>
	  //<<Walk to MRCA, Pa.~\ref{pa:tdb}>>
	  return mrca
  }
#+end_src
#+begin_src latex
  Lines are stored by mapping integers to booleans. The initial set of
  lines is the given set of taxa.
#+end_src
#+begin_src go <<Construct set of lines, Pa.~\ref{pa:tdb}>>=
...
#+end_src
#+begin_src latex
  The descendants are counted by mapping integers to integers. Each
  entry in the map has one descendant, itself.
#+end_src
#+begin_src go <<Construct descendants counter, Pa.~\ref{pa:tdb}>>=
...
#+end_src
#+begin_src latex
  We keep walking toward the root as long as we haven't found the
  MRCA. In each round of climbing we examine each line.
#+end_src
#+begin_src go <<Walk to MRCA, Pa.~\ref{pa:tdb}>>=
  found := false
  fmt.Printf("len(taxa): %d\n", len(taxa))
  for !found {
	  for l, _ := range lines {
		  //<<Examine line, Pa.~\ref{pa:tdb}>>
	  }
  }
#+end_src
#+begin_src latex
  We replace the current child line by its parent. We also increment the
  parent's number of descendants by the child's. If the parent has as
  many descendants as there are taxa, we've found the MRCA and break. If
  not, we replace the child line by its parent. We also remember that
  the current line might be visited repeatedly, so we set its
  descendants counter to zero to avoid double-counting.
#+end_src
#+begin_src go <<Examine line, Pa.~\ref{pa:tdb}>>=
  fmt.Printf("examining %d; ", l)
  p := t.Parent(l)
  desc[p] += desc[l]
  fmt.Printf("its parent %d has %d descendants\n", p, desc[p])
  if desc[p] == len(taxa) {
	  mrca = p
	  found = true
	  break
