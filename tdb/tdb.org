...
of the table returned by the query.c'
#+end_export
...
starting point of the new taxon is the offset. The end point is the
starting point plus the limit or the end of the slice, whichever is
smaller. We check that these bounds are within the slice in hand and
then return the corresponding taxon-IDs.
#+end_export
