...
#+end_src
#+begin_src latex
  For each parent we reach, we increment its descendants count by that
  of its children and check whether the number of its descendants is
  greater or equal to the number of taxa submitted. If yes, we've found
  the MRCA and return it.
#+end_src
