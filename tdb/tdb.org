...
#+end_src
#+begin_src latex
  \section{\ty{NewTaxonomyDB}}
  ! The function \ty{NewTaxonomyDB} takes as parameters the names of the
  ! six input files from which we construct the database, and the name
  ! of the database. The six input files are nodes, names, merged,
  ! images, genbank accessions, and refseq accessions. The function
  ! opens these files, opens a new database, and constructs the
  ! database.
#+end_src
...
#+end_src
#+begin_src latex
  We open the six input files and make sure they are eventually closed
  again.
#+end_src
...
#+end_src
#+begin_export latex
Figure~\ref{fig:db} shows four primary tables in our database,
\ty{taxon}, \ty{genome}, \ty{genome\_count}, and \ty{image}. In
addition, we implement the many to may relationship between \ty{taxon}
and \ty{image} as the auxiliary table \ty{tax2ima}.
#+end_export
#+begin_src go <<Construct tables, Pa. \ref{pa:tdb}>>=
  ...
  //<<Construct table \ty{image}, Pa. \ref{pa:tdb}>>
  //<<Construct table \ty{tax2ima}, Pa. \ref{pa:tdb}>>
#+end_src
#+begin_src latex
  ...
  are currently 2.9 million taxa, and this number is bound to grow, we
...
#+end_src
#+begin_export latex
Table \ty{image} has attributes \ty{image\_id}, which is also the
primary key,\ty{url}, and \ty{attribution}.
#+end_export
#+begin_src go <<Construct table \ty{image}, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_export latex
Table \ty{tax2ima} has two attributes, \ty{taxid} and \ty{image\_id},
which together form the primary key, and which we index
individually. \ty{taxid} and \ty{image\_id} are also foreign keys into
\ty{taxon} and \ty{image}, respectively.
#+end_export
#+begin_src go <<Construct table \ty{tax2ima}, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_src latex
  Since \ty{genome}, \ty{genome\_count}, and \ty{tax2ima} have a foreign
  key referring to \ty{taxon}, we load \ty{taxon} first. There's a snag
  here, though. The score of a taxon can only be determined once the
  genome counts have been determined. These in turn depend on the
  entries in table \ty{genome}. Hence we load \ty{genomes} after
  \ty{taxon}, followed by \ty{genome\_count}. Then we return to the
  scores, which we calculate, adjust, and enter into the table
  \ty{taxon}. We end by loading the two tables dealing with images,
  \ty{image} and \ty{tax2ima}. Since \ty{tax2ima} refers to \ty{image},
  we start with \ty{image}.
#+end_src
#+begin_src go <<Load tables, Pa. \ref{pa:tdb}>>=
  ...
  //<<Calculate scores, Pa. \ref{pa:tdb}>>
  //<<Adjust scores, Pa. \ref{pa:tdb}>>
  //<<Enter score in table \ty{taxon}, Pa. \ref{pa:tdb}>>
  //<<Load table \ty{image}, Pa. \ref{pa:tdb}>>
  //<<Load table \ty{tax2ima}, Pa. \ref{pa:tdb}>>
#+end_src
#+begin_src latex
  To load \ty{taxon}, we simulate a join of \ty{nodes.dmp} and
  \ty{names.dmp} on taxon-ID. For this we create the type \ty{taxon}
  with the same six fields as the six attributes in
...
#+begin_src latex
  ...
  scanner. We skip header rows and split each table row into its
  constituent fields, which we check. The first field is the accession,
  which we reduce to its core by calling the as yet unwritten function
  \ty{coreAcc}. The genome is constructed by calling \ty{fields2genome},
  also still unwritten. Then we insert the genome into the map as value
  to the core accession.
#+end_src
#+begin_src go <<Store refseq genomes, Pa. \ref{pa:tdb}>>=
  scanner = bufio.NewScanner(rf)
  for scanner.Scan() {
	  row := scanner.Text()
	  //<<Skip headers, Pa. \ref{pa:tdb}>>
	  fields := strings.Split(row, "\t")
	  //<<Check fields, Pa. \ref{pa:tdb}>>
	  k := coreAcc(fields[0])
...
#+end_src
#+begin_export latex
Headers start with a hash, and we skip them.
#+end_export
#+begin_src go <<Skip headers, Pa. \ref{pa:tdb}>>=
  if row[0] == '#' {
	  continue
  }
#+end_src
#+begin_export latex
Each row should consist of 38 fields. If not, something is wrong. So
we skip it with a warning.
#+end_export
#+begin_src go <<Check fields, Pa. \ref{pa:tdb}>>=
  n := len(fields)
  if n != 38 {
...
#+end_src
#+begin_export latex
Like we did with refseq accessions, we iterate over the rows of
genbank accessions. We skip the headers, split the non-header lines
into fields and check them. Then we store any new genome we might
find.
#+end_export
#+begin_src go <<Store genbank genomes, Pa. \ref{pa:tdb}>>=
  ...
  for scanner.Scan() {
	  ...
	  fields := strings.Split(row, "\t")
	  //<<Check fields, Pa. \ref{pa:tdb}>>
	  //<<Store new genome, Pa. \ref{pa:tdb}>>
  }
#+end_src
#+begin_export latex
A new genbank genome is one that has no entry in our genomes map yet.
#+end_export
#+begin_src go <<Store new genome, Pa. \ref{pa:tdb}>>=
  k := coreAcc(fields[0])
  if genomes[k] == nil {
	  g := fields2genome(fields)
	  genomes[k] = g
...
#+end_src
#+begin_export latex
We calculate the initial scores of teach taxon as the sum of its
recursive genome counts.
#+end_export
#+begin_src go <<Calculate scores, Pa. \ref{pa:tdb}>>=
  for _, taxon := range taxa {
...
#+end_src
#+begin_export latex
To adjust the scores, we first find their maximum, then we add that to
the taxa with common names.
#+end_export
#+begin_src go <<Adjust scores, Pa. \ref{pa:tdb}>>=
  //<<Calculate maximum score, Pa. \ref{pa:tdb}>>
  //<<Add maximum score to commoners, Pa. \ref{pa:tdb}>>
#+end_src
#+begin_export latex
We iterate over the taxa to find their maximum score.
#+end_export
#+begin_src go <<Calculate maximum score, Pa. \ref{pa:tdb}>>=
  max := 0.0
  for _, taxon := range taxa {
	  if taxon.score > max {
		  max = taxon.score
...
#+end_src
#+begin_export latex
If a taxon has a common name, we increment its scores by the maximum.
#+end_export
#+begin_src go <<Add maximum score to commoners, Pa. \ref{pa:tdb}>>=
  for _, taxon := range taxa {
	  if taxon.comName != "" {
		  taxon.score += max
	  }
...
#+end_src
#+begin_export latex
We enter the scores using a transaction that we prepare, before doing
the actual updating of the table \ty{taxon}. Then we commit the
transaction and close it again.
#+end_export
...
#+end_src
#+begin_export latex
We iterate over the taxa to update the score in their database entry.
#+end_export
...
#+end_src
#+begin_export latex
To load the table \ty{image}, we read the images and insert them.
#+end_export
#+begin_src go <<Load table \ty{image}, Pa. \ref{pa:tdb}>>=
  //<<Store images, Pa. \ref{pa:tdb}>>
  //<<Insert images, Pa. \ref{pa:tdb}>>
#+end_src
#+begin_export latex
We store an image in a struct with four fields, \ty{ID}, \ty{Url},
\ty{Attribution}, and \ty{taxids}. The first three fields are public,
as we shall use them to return images from method \ty{Images}. The
taxids attached to an image are only used in the construction of table
\ty{tax2ima} below, so we make them private.
#+end_export
#+begin_src go <<Types, Pa. \ref{pa:tdb}>>=
  type Image struct {
	  Id int
...
#+end_src
#+begin_export latex
We iterate over the file of images, split its fields, and store each
individual image in a slice of images.
#+end_export
#+begin_src go <<Store images, Pa. \ref{pa:tdb}>>=
  images := []Image{}
  ...
  for scanner.Scan() {
	  ...
	  fields := strings.Split(row, "\t|\t")
	  var image Image
	  //<<Store individual image, Pa. \ref{pa:tdb}>>
	  images = append(images, image)
  }
#+end_src
#+begin_export latex
For a given image we store its ID, URL, attribution, and set of
taxids.
#+end_export
#+begin_src go <<Store individual image, Pa. \ref{pa:tdb}>>=
  //<<Store image ID, Pa. \ref{pa:tdb}>>
  //<<Store image URL, Pa. \ref{pa:tdb}>>
  //<<Store image attribution, Pa. \ref{pa:tdb}>>
  //<<Store image taxids, Pa. \ref{pa:tdb}>>
#+end_src
#+begin_export latex
The image ID is the first field in the input row, and we convert it
from string to integer.
#+end_export
#+begin_src go <<Store image ID, Pa. \ref{pa:tdb}>>=
  id, err := strconv.Atoi(fields[0])
  util.Check(err)
  image.Id = id
#+end_src
#+begin_export latex
The image URL is in the third field.
#+end_export
#+begin_src go <<Store image URL, Pa. \ref{pa:tdb}>>=
  image.Url = fields[2]
#+end_src
#+begin_export latex
The attribution is in the fifth field.
#+end_export
#+begin_src go <<Store image attribution, Pa. \ref{pa:tdb}>>=
  image.Attribution = fields[4]
#+end_src
#+begin_export latex
The taxon IDs of the image are stored as a blank-separated string in
the eighth and last field of the row. This last field is terminated by
\verb+\t|+, which we chop off before converting the taxids from
strings to integers.
#+end_export
#+begin_src go <<Store image taxids, Pa. \ref{pa:tdb}>>=
  tstr := fields[7]
...
#+end_src
#+begin_export latex
We have read the images, so we are ready to insert them into their
table using the usual transaction executed on a prepared statement.
#+end_export
#+begin_src go <<Insert images, Pa. \ref{pa:tdb}>>=
  tx, err = db.Begin()
  util.Check(err)
  ...
  stmt, err = tx.Prepare(sqlStmt)
  util.Check(err)
  //<<Insert rows into table \ty{image}, Pa. \ref{pa:tdb}>>
  tx.Commit()
  stmt.Close()
#+end_src
#+begin_export latex
We iterate over the images and insert each one as a row into table
\ty{image}.
#+end_export
#+begin_src go <<Insert rows into table \ty{image}, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_export latex
The data we need to load table \ty{tax2ima} is already contained in
our slice of images. So we can directly construct the transaction for
inserting the rows into that table.
#+end_export
#+begin_src go <<Load table \ty{tax2ima}, Pa. \ref{pa:tdb}>>=
  tx, err = db.Begin()
  util.Check(err)
  ...
  stmt, err = tx.Prepare(sqlStmt)
  util.Check(err)
  //<<Insert rows into table \ty{ima2tax}, Pa. \ref{pa:tdb}>>
  tx.Commit()
  stmt.Close()
#+end_src
#+begin_export latex
We iterate over the images, and for each image we iterate over the
associated taxids to create the image/taxid pairs we insert into
\ty{tax2ima}.
#+end_export
#+begin_src go <<Insert rows into table \ty{ima2tax}, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_export latex
\section{\ty{OpenTaxonomyDBcheck}}
!\ty{OpenTaxonomyDBcheck} takes the name of a database and returns a
!connection to that database. If the database does not exist,
!\ty{OpenTaxonomyDB} returns an error.

We implement \ty{OpenTaxonomyDBcheck} by checking for the existence of
the database file, before we open and return the database.
#+end_export
#+begin_src go <<Functions, Pa. \ref{pa:tdb}>>=
...
#+begin_src latex
  ...
  !an error. It wraps \ty{AccessionsContext} which implements the just
  !mentioned behavior.

  \ty{Accessions} is a wrapper for \ty{AccessionsContext}.
#+end_src
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+begin_src latex
  ...
  We call \ty{NameContext} with the default context.
#+end_src
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+begin_src latex
  ...
  We call \ty{RankContext} with the default context.
#+end_src
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+begin_src latex
  ...
  We call \ty{ParentContext} with the default context.
#+end_src
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+begin_src latex
  ...
  We call \ty{ChildrenContext} with the default context.
#+end_src
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_src latex
  We define the children query, execute it, and extract the children.
#+end_src
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+begin_src latex
  ...
  We call \ty{SubtreeContext} with the default context.
#+end_src
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_src latex
  In \ty{traverseSubtree} we append the incoming node to the slice of
  taxa and increment the level. If the current level isn't greater than
  the maximum level, we get the node's children and iterate over them.
#+end_src
#+begin_src go <<Functions, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_export latex
We the children of the current node and check the error.
#+end_export
#+begin_src go <<Get children, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_src latex
  \section{\ty{SubtreeLevel}}
  ! The method \ty{SubtreeLevel} returns all taxa in a subtree up to a
  ! maximum level, including its root, and an error.

  We call \ty{SubtreeLevelContext} with the default context.
#+end_src
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+begin_src latex
  ...
  ! returns the corresponding taxon-IDs and an error. A negative limit
  ! means no limit.

  We call \ty{TaxidsContext} with the default context.
#+end_src
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_src latex
  We declare and execute the query for taxon-IDs, extract the IDs, and
  return them.
#+end_src
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_src latex
  We declare a template for the taxids query, where we get the taxon-IDs
  and scores for the taxa whose names match the given name.
#+end_src
#+begin_src go <<Query templates, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_src latex
  To extract the taxon-IDs, we first extract the taxa. The we sort them by
  score and apply the limit and the offset.
#+end_src
#+begin_src go <<Extract taxids, Pa. \ref{pa:tdb}>>=
  //<<Extract taxon-IDs, Pa. \ref{pa:tdb}>>
  //<<Sort taxa, Pa. \ref{pa:tdb}>>
  //<<Apply limit and offset, Pa. \ref{pa:tdb}>>
#+end_src
#+begin_export latex
We store the taxa in a slice that we fill by iterating over the rows
of the table returned by the query.
#+end_export
#+begin_src go <<Extract taxon-IDs, Pa. \ref{pa:tdb}>>=
  taxa := []taxon{}
  for rows.Next() {
  	t := taxon{}
...
#+end_src
#+begin_export latex
We sort the taxa by their score as primary key and taxon-ID as
secondary key.
#+end_export
#+begin_src go <<Sort taxa, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_export latex
We reslice \ty{taxa} according to the limit and the offset. The
starting point of the new \ty{taxa} is the offset. The end point is
the starting point plus the limit or the end of the slice, whichever
is smaller. We check that these bounds are within the slice in hand
and then store the corresponding taxon-IDs.
#+end_export
#+begin_src go <<Apply limit and offset, Pa. \ref{pa:tdb}>>=
  s := offset
  e := s + limit
...
#+begin_src latex
  ...
  We call \ty{CommonTaxidsContext} with the default context.
#+end_src
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_src latex
  We declare and execute the query for taxon-IDs from common and
...
#+begin_src latex
  ...
  We call \ty{MRCAContext} with the default context.
#+end_src
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_src latex
  To design our search for the most recent common ancestor, MRCA, we
  look at Figure~\ref{fig:tax}, where the MRCA of taxa 4 and 7 is 3. It
  is important to note that the input taxa may be in an
  ancestor/descendant relationship. For example, in Figure~\ref{fig:tax}
  the MRCA of taxa 4, 7, and 5 is also 3. Such an ancestor/descendant
  relationships may lead to the inclusion of the MRCA in the input. In
  our example the MRCA of taxa 4, 7, 5, and 3 is again 3. In the trivial
  case where only a single taxon was submitted, that is the MRCA.

  We begin by checking the IDs supplied and then search for the MRCA. If
  we haven't found one, we return -1.
#+end_src
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+begin_src latex
  ...
  science~\cite[ch. 3]{ohl13:bio}. However, instead of using a general
  solution to the problem, we construct a simpler solution that involves
  tracing the line of descent for each input taxon.

  To be more precise, consider $n$ input taxa. For the first $n-1$ of
  them we climb the tree to the root and increment a descendants counter
  for each node. Then we climb from the last taxon toward the root; the
  first descendants counter equal to the number of input taxa belongs to
  the MRCA.

  So we construct a descendants counter and initialize it. Then we climb
  the tree for the first $n-1$ taxa, before we deal with the last taxon.
#+end_src
#+begin_src go <<Search for MRCA, Pa. \ref{pa:tdb}>>=
  desc := make(map[int]int)
  //<<Initialize descendats counts, Pa. \ref{pa:tdb}>>
  //<<Climb tree for first $n-1$ taxa, Pa. \ref{pa:tdb}>>
  //<<Deal with last taxon, Pa. \ref{pa:tdb}>>
#+end_src
#+begin_export latex
We initialize the descendants counts of each input taxon to 1.
#+end_export
#+begin_src go <<Initialize descendats counts, Pa. \ref{pa:tdb}>>=
  for _, id := range ids {
  	desc[id] = 1
  }
#+end_src
#+begin_export latex
We climb the tree for the first $n-1$ taxa and increment the
descendants counts for every taxon we meet on the path to the root.
#+end_export
#+begin_src go <<Climb tree for first $n-1$ taxa, Pa. \ref{pa:tdb}>>=
  n := len(ids)
...
#+end_src
#+begin_export latex
We have reached the last taxon. It might already be the MRCA. If not,
we climb the tree one last time. If we didn't find the MRCA, we throw
an error.
#+end_export
#+begin_src go <<Deal with last taxon, Pa. \ref{pa:tdb}>>=
  v := ids[n-1]
  if desc[v] == n {
  	mrca = v
  } else {
  	//<<Climb tree for last taxon, Pa. \ref{pa:tdb}>>
  }
  if mrca == -1 {
...
#+end_src
#+begin_export latex
We climb toward the root and for every parent ask, whether it is the
MRCA. If so, we store it and break from the loop.
#+end_export
#+begin_src go <<Climb tree for last taxon, Pa. \ref{pa:tdb}>>=
  p, e := t.Parent(v)
  util.Check(e)
  for p != v {
  	desc[p]++
...
#+begin_src latex
  ...
  ! returns the assembly level and an error.

We call \ty{LevelContext} with the default context.
#+end_src
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_src latex
  We define the level query, execute it, and extract the level before
  we return it.
#+end_src
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
We call \ty{NumGenomesContext} with the default context.
#+end_export
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_export latex
We construct the query for the number of genomes, execute it, and
extract the result.
#+end_export
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
We call \ty{NumGenomesRecContext} with the default context.
#+end_export
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+begin_export latex
\section{\ty{IsLeaf}}
!The method \ty{IsLeaf} takes as argument a taxon-ID and returns true
!if the taxon has no children, i. e. is a leaf, false otherwise. It
!also returns an error.

We call \ty{IsLeafContext} with the default context.
#+end_export
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_export latex
We implement the method by counting the children associated with the
incoming taxon-ID. If that number is zero, we return true, false
otherwise.
#+end_export
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
  func (d *TaxonomyDB) IsLeafContext(
	  ...
	  numChildren := 0
	  var err error
	  //<<Construct leaf query, Pa. \ref{pa:tdb}>>
	  //<<Execute leaf query, Pa. \ref{pa:tdb}>>
	  //<<Extract number of children, Pa. \ref{pa:tdb}>>
	  if numChildren == 0 {
		  return true, nil
	  }
	  return false, nil
...
#+end_src
#+begin_export latex
We construct the query for counting the children of the given
taxon-ID.
#+end_export
#+begin_src go <<Construct leaf query, Pa. \ref{pa:tdb}>>=
  q := "select count(taxid) from taxon " +
	  "where parent=?"
#+end_src
#+begin_export latex
We execute the leaf query and bail if there was an error.
#+end_export
#+begin_src go <<Execute leaf query, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_export latex
We extract the number of children and bail if there was an error.
#+end_export
#+begin_src go <<Extract number of children, Pa. \ref{pa:tdb}>>=
  row.Next()
...
#+begin_export latex
\section{\ty{Images}}
!The method \ty{Images} takes as argument a taxon-ID and returns all
!images associated with the taxon. It also returns an error.
#+end_export
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_export latex
  We begin by defining a query, executing it and lastly getting the images.
#+end_export
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
  func (d *TaxonomyDB) ImagesContext(
	  ...
	  images := []Image{}
	  var err error
	  //<<Construct image query, Pa. \ref{pa:tdb}>>
	  //<<Execute image query, Pa. \ref{pa:tdb}>>
	  //<<Extract images, Pa. \ref{pa:tdb}>>
	  return images, err
  }
#+end_src
#+begin_export latex
The table \ty{tax2ima} links taxa to images, while the actual image
information is stored in \ty{image}. We join these two tables on their
one common attribute, \ty{image\_id}, and select the rows for the
incoming taxid.
#+end_export
#+begin_src go <<Construct image query, Pa. \ref{pa:tdb}>>=
  q := "select image_id, url, attribution " +
	  "from image natural join tax2ima " +
	  "where taxid=?"
#+end_src
#+begin_export latex
We execute the image query and check the error returned.
#+end_export
#+begin_src go <<Execute image query, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_export latex
We iterate over the rows returned by our query and for each row store
an image.
#+end_export
#+begin_src go <<Extract images, Pa. \ref{pa:tdb}>>=
  for rows.Next() {
	  image := Image{}
...
#+end_src
#+begin_src latex
  \section{\ty{AccessionTaxid}}
  !The method \ty{AccessionTaxid} takes as argument a genome accession
  !and returns the corresponding taxon-ID and an error.

  We call \ty{AccessionTaxidContext} with the default context.
#+end_src
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_src latex
  We define the query for the taxon-ID, execute it, and extract the
  taxon-ID before we return it.
#+end_src
#+begin_src go <<Methods, Pa. \ref{pa:tdb}>>=
  func (t *TaxonomyDB) AccessionTaxidContext(
	  ...
	  var err error
	  taxid := 0
	  //<<Execute taxid query, Pa. \ref{pa:tdb}>>
	  //<<Extract taxid, Pa. \ref{pa:tdb}>>
	  return taxid, err
  }
#+end_src
#+begin_src latex
  We define a template for the taxid query.
#+end_src
#+begin_src go <<Query templates, Pa. \ref{pa:tdb}>>=
  var taxidT = "select taxid from genome where accession=?"
#+end_src
#+begin_src latex
  We execute the taxid query, check the error, and make sure the result
  stream we just opened is eventually closed again.
#+end_src
#+begin_src go <<Execute taxid query, Pa. \ref{pa:tdb}>>=
...
#+begin_src latex
  We scan the first row in the table and check the error returned.
#+end_src
#+begin_src go <<Extract taxid, Pa. \ref{pa:tdb}>>=
  rows.Next()
  err = rows.Scan(&taxid)
  if err != nil {
	  m := "couldn't find the taxon of " +
...
#+end_src
#+begin_export latex
Having tested the functions, we test the methods. We do this in four
lots, depending on which table to access, \ty{genome}, \ty{accession},
\ty{genome\_count}, or \ty{image}.
#+end_export
#+begin_src go <<Test methods, Pa. \ref{pa:tdb}>>=
  ...
  //<<Test access to \ty{image}, Pa. \ref{pa:tdb}>>
#+end_src
#+begin_export latex
We test the twelve methods that access \ty{taxon}, \ty{Name},
\ty{CommonName}, \ty{Rank}, \ty{Parent}, \ty{Children}, \ty{Subtree},
\ty{SubtreeLevel}, \ty{MRCA}, and \ty{NumTaxa}, \ty{Taxids},
\ty{CommonTaxids}, and \ty{IsLeaf}.
#+end_export
...
#+end_src
#+begin_export latex
The \emph{Hominidae} subtree up to level 2 contains 12 nodes.
#+end_export
#+begin_src go <<Test \ty{SubtreeLevel}, Pa. \ref{pa:tdb}>>=
...
#+begin_src latex
  ...
  \emph{homo sapiens}, but limit the output to the middle two. Notice
  the lower-case ``h'' in \emph{homo}, which SQL matches to the
  upper-case ``H'' in the actual names.
#+end_src
...
#+end_src
#+begin_export latex
We test \ty{IsLeaf}, we query for \emph{Homo sapiens}, which we know
is not a leaf.
#+end_export
#+begin_src go <<Test \ty{IsLeaf}, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_export latex
We test the four Methods that access \ty{genome}, \ty{Accessions},
\ty{Level}, \ty{FilterAccessions}, and \ty{AccessionTaxid}.
#+end_export
#+begin_src go <<Test access to \ty{genome}, Pa. \ref{pa:tdb}>>=
  ...
  //<<Test \ty{AccessionTaxid}, Pa. \ref{pa:tdb}>>
#+end_src
#+begin_export latex
...
#+end_src
#+begin_export latex
To test the \ty{AccessionTaxid}, we make sure we get taxid 730 for
accession \ty{GCF\_001465235.1}.
#+end_export
#+begin_src go <<Test \ty{AccessionTaxid}, Pa. \ref{pa:tdb}>>=
  acc = "GCF_001465235.1"
...
#+end_src
#+begin_export latex
There is only one method for accessing table \ty{image},
\ty{Images}. For human it returns a single item attributed to ``All of
Us Initiative''.
#+end_export
#+begin_src go <<Test access to \ty{image}, Pa. \ref{pa:tdb}>>=
...
#+end_src
#+begin_export latex
We also test what happens when we try to retrieve the pictures for a
non-existent taxon. In that case the length of the image slice
...
#+end_src
#+begin_export latex
As a next step we test for injection safety. This really only works
for methods that execute queries and take strings as arguments.
Given this constraint we test these six methods:
\ty{Taxids},
\ty{CommonTaxids},
\ty{Level},
\ty{NumGenomes},
...
#+end_export
#+begin_src go <<Test methods, Pa. \ref{pa:tdb}>>=
//<<Test injection \ty{Taxids}, Pa. \ref{pa:tdb}>>
//<<Test injection \ty{CommonTaxids}, Pa. \ref{pa:tdb}>>
//<<Test injection \ty{Level}, Pa. \ref{pa:tdb}>>
//<<Test injection \ty{NumGenomes}, Pa. \ref{pa:tdb}>>
//<<Test injection \ty{NumGenomesRec}, Pa. \ref{pa:tdb}>>
//<<Test injection \ty{AccessionTaxid}, Pa. \ref{pa:tdb}>>
#+end_src
#+begin_export latex
We begin with \ty{Taxids}. \ty{Taxids} uses the string argument to
filter names with the \ty{like} operator. In order to check for injectability
we need to define a string that, somewhat, escapes the flow of the query,
execute it and check the results.
#+end_export
#+begin_src go <<Test injection \ty{Taxids}, Pa. \ref{pa:tdb}>>=
  //<<Define injection string, Pa. \ref{pa:tdb}>>
  //<<Execute injection query \ty{Taxids}, Pa. \ref{pa:tdb}>>
  //<<Check injection results \ty{Taxids}, Pa. \ref{pa:tdb}>>
#+end_src
#+begin_export latex
Our goal is to escape the filtering done by the \ty{like} operator and return
an unfiltered result instead.
We use two test strings, one for queries where the token is string-enclosed and one for non-enclosed tokens.
The non-enclosed token could be something along the lines of \ty{'' or 1=1}. Here \ty{''} catches the \ty{like}
operator. This allows us to add another condition, via the \ty{or} operator, which
always evaluates to \ty{true}. Thus we get an unfiltered result. The enclosed token looks similar and
might be something like this: \ty{' or 1=1 or '}. Here the first apostrophe breaks us out of the string
literal. This again allows adding new conditions. The second apostrophe binds with the enclosing
character of the original query, making the injected query a valid statement.
#+end_export
#+begin_src go <<Define injection string, Pa. \ref{pa:tdb}>>=
  strNE := "'' or 1=1"
  strE := "' or 1=1 or '"
#+end_src
#+begin_export latex
Now we execute the queries by calling \ty{Taxids}.
#+end_export
#+begin_src go <<Execute injection query \ty{Taxids}, Pa. \ref{pa:tdb}>>=
  taxids1, _ := taxdb.Taxids(strNE, 10, 0)
  taxids2, _ := taxdb.Taxids(strE, 10, 0)
#+end_src
#+begin_export latex
We expect that \ty{Taxids} is injection safe meaning both queries find
no results.
#+end_export
#+begin_src go <<Check injection results \ty{Taxids}, Pa. \ref{pa:tdb}>>=
  g = len(taxids1)
  w = 0
  if g != w {
	  t.Errorf("\nget:%d\nwant:%d\n", g, w)
...
#+end_src
#+begin_export latex
\ty{CommonTaxids} works just like \ty{CommonTaxids}, so we proceed as we did before.
#+end_export
#+begin_src go <<Test injection \ty{CommonTaxids}, Pa. \ref{pa:tdb}>>=
//<<Execute injection query \ty{CommonTaxids}, Pa. \ref{pa:tdb}>>
//<<Check injection results \ty{CommonTaxids}, Pa. \ref{pa:tdb}>>
#+end_src
#+begin_export latex
Now we execute the queries by calling \ty{Taxids}.
#+end_export
#+begin_src go <<Execute injection query \ty{CommonTaxids}, Pa. \ref{pa:tdb}>>=
  ctaxids1, _ := taxdb.CommonTaxids(strNE, 10, 0)
  ctaxids2, _ := taxdb.CommonTaxids(strE, 10, 0)
#+end_src
#+begin_export latex
We expect that \ty{CommonTaxids} is injection safe meaning both queries find
no results.
#+end_export
#+begin_src go <<Check injection results \ty{CommonTaxids}, Pa. \ref{pa:tdb}>>=
  g = len(ctaxids1)
  w = 0
  if g != w {
	  t.Errorf("\nget:%d\nwant:%d\n", g, w)
...
#+end_src
#+begin_export latex
\ty{Level} does not return a list of elements, so we need to think of a new
injection. Here we could try to escape the query to create an unintended return value.
This value is going to be the string literal \ty{'injected'}.
#+end_export
#+begin_src go <<Test injection \ty{Level}, Pa. \ref{pa:tdb}>>=
//<<Define injection string \ty{Level}, Pa. \ref{pa:tdb}>>
//<<Execute injection query \ty{Level}, Pa. \ref{pa:tdb}>>
//<<Check injection results \ty{Level}, Pa. \ref{pa:tdb}>>
#+end_src
#+begin_export latex
We do an enclosed and a non enclosed version again.
#+end_export
#+begin_src go <<Define injection string \ty{Level}, Pa. \ref{pa:tdb}>>=
  strNE = "1;select 'injected'"
  strE = "';select 'injected' where '1"
#+end_src
#+begin_export latex
We execute the queries by calling \ty{Level}.
#+end_export
#+begin_src go <<Execute injection query \ty{Level}, Pa. \ref{pa:tdb}>>=
  level1, _ := taxdb.Level(strNE)
  level2, _ := taxdb.Level(strE)
#+end_src
#+begin_export latex
We expect that \ty{Level} is injection safe, meaning both queries find
no results.
#+end_export
#+begin_src go <<Check injection results \ty{Level}, Pa. \ref{pa:tdb}>>=
  wStr := ""
  if level1 != wStr {
	  t.Errorf("\nget:%s\nwant:%s\n", level1, wStr)
  }
...
#+end_src
#+begin_export latex
\ty{NumGenomes} works again similar to \ty{Level}, in the context of injections.
In this case we need to return an integer, so we again escape the query
and return \ty{-1}.
#+end_export
#+begin_src go <<Test injection \ty{NumGenomes}, Pa. \ref{pa:tdb}>>=
//<<Define injection string \ty{NumGenomes}, Pa. \ref{pa:tdb}>>
//<<Execute injection query \ty{NumGenomes}, Pa. \ref{pa:tdb}>>
//<<Check injection results \ty{NumGenomes}, Pa. \ref{pa:tdb}>>
#+end_src
#+begin_export latex
  We do an enclosed and a non enclosed version again.
#+end_export
#+begin_src go <<Define injection string \ty{NumGenomes}, Pa. \ref{pa:tdb}>>=
  strNE = ";select -1"
  strE = "';select -1 where '1"
#+end_src
#+begin_export latex
We execute the queries by calling \ty{NumGenomes}.
#+end_export
#+begin_src go <<Execute injection query \ty{NumGenomes}, Pa. \ref{pa:tdb}>>=
  ng1, _ := taxdb.NumGenomes(-1, strNE)
  ng2, _ := taxdb.NumGenomes(-1, strE)
#+end_src
#+begin_export latex
We expect that \ty{NumGenomes} is injection safe, meaning both queries return
\ty{0} as there is neither a taxon with id \ty{-1} nor a level with that cryptic
name we defined above.
#+end_export
#+begin_src go <<Check injection results \ty{NumGenomes}, Pa. \ref{pa:tdb}>>=
  wi := 0
  if ng1 != wi {
	  t.Errorf("\nget:%d\nwant:%d\n", ng1, wi)
  }
...
#+end_src
#+begin_export latex
We can use the exact same method and string for \ty{NumGenomesRec}.
#+end_export
#+begin_src go <<Test injection \ty{NumGenomesRec}, Pa. \ref{pa:tdb}>>=
//<<Execute injection query \ty{NumGenomesRec}, Pa. \ref{pa:tdb}>>
//<<Check injection results \ty{NumGenomesRec}, Pa. \ref{pa:tdb}>>
#+end_src
#+begin_export latex
We execute the queries by calling \ty{NumGenomesRec}.
#+end_export
#+begin_src go <<Execute injection query \ty{NumGenomesRec}, Pa. \ref{pa:tdb}>>=
  ng1, _ = taxdb.NumGenomesRec(-1, strNE)
  ng2, _ = taxdb.NumGenomesRec(-1, strE)
#+end_src
#+begin_export latex
We expect that \ty{NumGenomesRec} is injection safe, meaning both queries return
\ty{0} as there is neither a taxon with id \ty{-1} nor a level with that cryptic
name we defined above.
#+end_export
#+begin_src go <<Check injection results \ty{NumGenomesRec}, Pa. \ref{pa:tdb}>>=
  wi = 0
  if ng1 != wi {
	  t.Errorf("\nget:%d\nwant:%d\n", ng1, wi)
  }
...
#+end_src
#+begin_export latex
Lastly we check for injections in \ty{AccessionTaxid}. We use the same schema as we
did with the previous two checks. We can reuse the injection strings as well.
#+end_export
#+begin_src go <<Test injection \ty{AccessionTaxid}, Pa. \ref{pa:tdb}>>=
//<<Execute injection query \ty{AccessionTaxid}, Pa. \ref{pa:tdb}>>
//<<Check injection results \ty{AccessionTaxid}, Pa. \ref{pa:tdb}>>
#+end_src
#+begin_export latex
We execute the queries by calling \ty{AccessionTaxid}.
#+end_export
#+begin_src go <<Execute injection query \ty{AccessionTaxid}, Pa. \ref{pa:tdb}>>=
  at1, _ := taxdb.AccessionTaxid(strNE)
  at2, _ := taxdb.AccessionTaxid(strE)
#+end_src
#+begin_export latex
We expect that \ty{AccessionTaxid} is injection safe, meaning both queries return
\ty{0} as there is no taxon with the cryptic name defined above.
#+end_export
#+begin_src go <<Check injection results \ty{AccessionTaxid}, Pa. \ref{pa:tdb}>>=
  wi = 0
  if at1 != wi {
	  t.Errorf("\nget:%d\nwant:%d\n", at1, wi)
  }
