#+begin_src latex
  \section{\ty{ants}}
  Given a taxon-ID, \ty{ants} returns the ancestor. It can also return
  \emph{all} ancestors.
...
#+begin_src go <<Set usage, Pr.~\ref{pr:ant}>>=
  ...
  p := "Get a taxon's ancestor or all of its ancestors."
  e := "ants -a 9606 tax.db"
  clio.Usage(u, p, e)
#+end_src
#+begin_src latex
  We import \ty{clio}.
...
#+end_src
#+begin_src latex
  Apart from the version option, we declare the option for printing all
  ancestors.
#+end_src
#+begin_src go <<Declare options, Pr.~\ref{pr:ant}>>=
  var optV = flag.Bool("v", false, "version")
  var optA = flag.Bool("a", false, "all ancestors")
#+end_src 
#+begin_src latex
  We import \ty{flag}.
#+end_src
...
#+end_src
#+begin_src latex
  We open a database connection and get the first ancestor. If the user
  opted to see all ancestors, we also get the remaining ancestors.
#+end_src
#+begin_src go <<Get ancestors, Pr.~\ref{pr:ant}>>=
  taxdb := tdb.OpenTaxonomyDB(db)
  //<<Get first ancestor, Pr.~\ref{pr:ant}>>
  if *optA {
	  //<<Get remaining ancestors, Pr.~\ref{pr:ant}>>
  }
#+end_src
#+begin_src latex
  We import \ty{tdb}.
...
#+end_src
#+begin_src latex
  We store the input taxon and get its first ancestor.
#+end_src
#+begin_src go <<Get first ancestor, Pr.~\ref{pr:ant}>>=
  var ants []int
...
#+end_src
#+begin_src latex
  We climb towards the root to collect the remaining ancestors.
#+end_src
#+begin_src go <<Get remaining ancestors, Pr.~\ref{pr:ant}>>=
  tid = a
  a = taxdb.Parent(tid)	
  for tid != a {
	  ants = append(ants, a)
...
#+end_src
#+begin_src latex
  We print the ancestors starting with the most remote. To make the
  result look nice, we print a table and lay it out with a
  \ty{tabwriter}.
#+end_src
#+begin_src go <<Print ancestors, Pr.~\ref{pr:ant}>>=
  w := tabwriter.NewWriter(os.Stdout, 0, 1, 2, ' ', 0)
  defer w.Flush()
  fmt.Fprintf(w, "# Back\tID\tName\n")
...
#+end_src
#+begin_src latex
  We run \ty{ants} starting from \emph{Homo sapiens} (9606), first with
  only a single ancestor, then with all ancestors.
#+end_src
#+begin_src go <<Construct tests, Pr.~\ref{pr:ant}>>=
  ...
  test = exec.Command("./ants", "-a", tid, db)
  tests = append(tests, test)
#+end_src
#+begin_src latex
