...
#+begin_src go <<ants.go>>=
  ...
  func Run() {
	  //<<Run function, Pr. \ref{pr:ant}>>
  }
#+end_src
#+begin_src latex
  \epr In the run function we set the program name and the usage,
  declare the options, parse the options, get the ancestors, and print
  them.
#+end_src
#+begin_src go <<Run function, Pr. \ref{pr:ant}>>=
  util.SetName("ants")
...
#+end_src
#+begin_src latex
  There's are three options, the version and remote execution and remote
  execution with a specific database.
#+end_src
...
#+begin_src latex
  ...
  program. We execute remotely if \ty{-r} is active. We extract
  the remaining arguments, check there are two of them, and store
  the taxon-ID and the database for local execution.
#+end_src
...
#+end_src
#+begin_src latex
  The remote db option enables remote execution of \ty{ants}. This uses the
  database specified via \ty{optR}. For this we need to call
  \ty{SendGetRequest}. We use the path: \ty{api/v2/programs/ants}.
#+end_src
...
#+begin_src latex
  ...
  \emph{Homo sapiens} (9606). We test this with the remote options as well.
#+end_src
