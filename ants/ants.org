...
#+end_src
#+begin_src latex
  There's are two options, the version and remote execution.
#+end_src
...
#+end_src
#+begin_src latex
  The remote switch enables remote execution of \ty{ants}.
  For this we need to call \ty{SendGetRequest}.
  The servers adress is:
  \ty{http://localhost:8080/api/v2/taxa/{taxon_id}/ancestors}.
#+end_src
...
#+end_src
#+begin_src latex
  We need to import \ty{strings}, when using the \ty{Join} function.
#+end_src
#+begin_src go <<Imports, Pr. \ref{pr:ant}>>=
  "strings"
#+end_src
