...
is based on a combination of a genome aligner and
\ty{bedtools}~\cite{qui10:bed}. Using these two programs,
\ty{findMacs} performs the following steps:
\begin{itemize}
...
The subtraction in equation~(\ref{eq:sub}) 
is implemented by
intersecting $r$ with $N$ using the aligner, and subtracting the
intersection from $r$ using \ty{bedtools}.
\[
M_1=r\setminus (r\cap N).
\]
An example of this is shown in Figure~\ref{fig:fin}A.
...
\begin{figure}
  ...
  \caption{Illustration of the subtraction (\textbf{A}) and
    ...
    targets, $t_1,t_2,t_3$, of which $t_1$ is the target
    representative, $r$; the dotted vertical lines in panel \textbf{B}
    indicate the expected result.}\label{fig:fin}
\end{figure}

The intersection in equation~(\ref{eq:int}) is implemented by
intersecting the other targets with $r$. Each intersection is then
subtracted from $r$ and the union of these differences is subtracted
from $M_1$ to give the final set of markers, $M_2$,
\[
M_2=M_1\setminus((r\setminus(r\cap t_2))\cup(r\setminus(r\cap
t_3))\cup...\cup(r\setminus(r\cap t_n))).
\]
Figure~\ref{fig:fin}B shows an example of this, starting from the
result in Figure~\ref{fig:fin}A, $M_1$.

Let's explore in more detail how \ty{bedtools} can be used to
implement equations~(\ref{eq:sub}) and~(\ref{eq:int}). For this we
store the intersections $I_1$, $I_2$, and $I_3$ from
Figures~\ref{fig:fin}A and B as BED intervals in files
\ty{findMacs/i[123].bed}. Bed intervals are half-open to the left, hence
\ty{i1.bed} contains $I_1=\{(0,1],(3,6],(8,10]\}$ written in three
      columns:
\begin{verbatim}
r	0	1
r	3	6
r	8	10
\end{verbatim}
In addition, there is also the interval $r=(0,10]$ in
  \ty{findMacs/r.bed}. We write a little script to demonstrate the
...
#+end_src
#+begin_export latex
\epr We subtract $I_1$ from $r$ to get $M_1$.
#+end_export
#+begin_src sh <<Subtraction, Pr. \ref{pr:bed}>>=
  bedtools subtract -a r.bed -b i1.bed > m1.bed
#+end_src
#+begin_export latex
For the intersection we first perform two subtractions and store the
differences in files \ty{d1.bed} and \ty{d2.bed}.
#+end_export
#+begin_src sh <<Intersection, Pr. \ref{pr:bed}>>=
  bedtools subtract -a r.bed -b i2.bed > d1.bed
  bedtools subtract -a r.bed -b i2.bed > d2.bed
#+end_src
#+begin_export latex
We form the union of the differences using the \ty{merge} function of
\ty{bedtools}. It takes as input a set of sorted intervals. We save
the result in the file \ty{x.bed}.
#+end_export
#+begin_src sh <<Intersection, Pr. \ref{pr:bed}>>=
  sort -k 2 -n d*.bed |
      bedtools merge > x.bed
#+end_src
#+begin_export latex
All that remains is to subtract $X$ from $M_1$ to get the final two
intervals, $M_2=\{(2,3], (7,8]\}$, which are the intervals indicated
    by the dotted vertical lines in Figure~\ref{fig:fin}B. At the end
    we remove the intermediate bed files again.
#+end_export
#+begin_src sh <<Intersection, Pr. \ref{pr:bed}>>=
  bedtools subtract -a m1.bed -b x.bed
  rm c[12].bed d[12].bed m1.bed x.bed
#+end_src
#+begin_export latex
\begin{verbatim}
r       2       3
...
#+begin_src sh <<Compare to neighbors with \ty{nucmer}, Pr. \ref{pr:fin}>>=
  ...
  nucmer --delta=$delta -t $tOpt $r $query
...
#+begin_src sh <<Compare to neighbors with \ty{mashmap}, Pr. \ref{pr:fin}>>=
  ...
  mashmap -s 100  -t $tOpt -q $query -r $r -o $paf
#+end_src
#+begin_export latex
...
#+begin_src sh <<Compare rep with \ty{nucmer}, Pr. \ref{pr:fin}>>=
  nucmer --delta=$delta -t $tOpt $r $target
  paftools.js delta2paf $delta > $paf
#+end_src
#+begin_export latex
...
#+begin_src sh <<Compare rep with \ty{mashmap}, Pr. \ref{pr:fin}>>=
  ...
  mashmap -s 100 -t $tOpt -r $r -q $target -o $paf
#+end_src
#+begin_export latex
We extract the target hits from the PAF file and sort them by the
sequence name and within a sequence by position.
#+end_export
