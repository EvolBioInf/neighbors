...
#+begin_src latex
  ...
  the suffix, and the remove option.
#+end_src
...
#+end_src
#+begin_src latex
  We construct four tests, one with default options, one where we set a
  prefix, one where we set a suffix, and one where we remove existing
  labels. The first three tests work on the unlabeled tree contained in
  \ty{test1.nwk}, the last on the labeled tree contained in
  \ty{test2.nwk}.
#+end_src
