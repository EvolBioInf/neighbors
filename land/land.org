#+begin_src latex
  ...
  nodes of a labeled tree and/or from its leaves. Removal of a
  particular set of labels leaves the remaining labels unchanged.

  \section*{Implementation}
  \ty{land} contains hooks for imports, functions, and the logic of the
  run function.  \bpr{land}{pr:lan}
#+end_src
...
#+end_src
#+begin_src latex
  \epr In the run function of \ty{land} we set the name of \ty{land}
...
#+end_src
#+begin_src latex
  When labeling a node, we distinguish between internal nodes and leaf
  nodes.
#+end_src
#+begin_src go <<Label node, Pr. \ref{pr:lan}>>=
  label := v.Label
  if v.Child != nil {
  	//<<Label internal node, Pr. \ref{pr:lan}>>
  } else {
  	//<<Label leaf node, Pr. \ref{pr:lan}>>
  }
  v.Label = label
#+end_src
#+begin_export latex
When labeling an internal node, we first ask, whether the user is
removing labels. If so, we set the label to the empty
string. Otherwise, we construct the new label and increment the label
counter, unless the user is removing leaf labels, in which case we
leave the label unchanged.
#+end_export
#+begin_src go <<Label internal node, Pr. \ref{pr:lan}>>=
  if ri {
  	label = ""
  } else if !rl {
...
#+end_src
#+begin_export latex
When labeling a leaf, we ask whether the user is removing leaf labels,
in which case we set the label to the empty string.
#+end_export
#+begin_src go <<Label leaf node, Pr. \ref{pr:lan}>>=
  if rl {
  	label = ""
  }
#+end_src
