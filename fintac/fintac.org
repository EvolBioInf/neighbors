...
takes as argument a slice of counts and returns the filled-in slice of
counts. It also takes the neighbor and target prefixes as arguments.
#+end_export
#+begin_src go <<Count nodes, Pr. \ref{pr:fit}>>=
  var counts []*count
  counts = traverseTree(tree, counts, *optN, *optT)
#+end_src
#+begin_export latex
A count is understood per node; so it consists of the ID of the node,
its label, the label of its parent, the number of neighbors and
targets in its subtree, and its split as defined in
equation~(\ref{eq:spl}).
#+end_export
...
its input node in order. This means, we analyze the current node, $v$,
before calling \ty{traverseTree} on its child and sibling.
#+end_export
#+begin_src go <<Functions, Pr. \ref{pr:fit}>>=
  func traverseTree(v *nwk.Node, counts []*count,
	  np, tp string) []*count {
	  if v == nil {
		  return counts
	  }
	  //<<Analyze $v$, Pr. \ref{pr:fit}>>
	  counts = traverseTree(v.Child, counts, np, tp)
	  counts = traverseTree(v.Sib, counts, np, tp)
	  return counts
  }
#+end_src
#+begin_export latex
...
#+begin_src go <<Analyze $v$, Pr. \ref{pr:fit}>>=
  if v.Child == nil {
	  ...
	  //<<Count nodes in subtree, Pr. \ref{pr:fit}>>
  }
#+end_src
#+begin_export latex
If the leaf label is neither target nor neighbor, something is wrong
and we bail with message.
#+end_export
#+begin_src go <<Check leaf label, Pr. \ref{pr:fit}>>=
  if !strings.HasPrefix(v.Label, np) &&
	  !strings.HasPrefix(v.Label, tp) {
...
#+end_src
#+begin_export latex
To count nodes in the subtree, we need another recursive function,
\ty{countNodes}.
#+end_export
#+begin_src go <<Count nodes in subtree, Pr. \ref{pr:fit}>>=
  c := new(count)
  c.id = v.Id
  c.label = v.Label
  c.parent = "-"
  if v.Parent != nil {
	  c.parent = v.Parent.Label
  }
  countNodes(v.Child, c, tp)
  counts = append(counts, c)
#+end_src
#+begin_export latex
Inside \ty{countNodes} we check whether we are dealing with a leaf, in
which case we classify it. Then we complete the recursion.
#+end_export
#+begin_src go <<Functions, Pr. \ref{pr:fit}>>=
  func countNodes(v *nwk.Node, c *count, tp string) {
	  if v == nil {
		  return
	  }
	  if v.Child == nil {
		  //<<Classify leaf, Pr. \ref{pr:fit}>>
	  }
	  countNodes(v.Child, c, tp)
	  countNodes(v.Sib, c, tp)
  }
#+end_src
#+begin_export latex
A leaf is either a target or a neighbor and we increment the
corresponding counter.
#+end_export
#+begin_src go <<Classify leaf, Pr. \ref{pr:fit}>>=
  if strings.HasPrefix(v.Label, tp) {
	  c.vt++
  } else {
	  c.vn++
  }
...
root, which are stored in the first entry of \ty{counts}. Having read
the totals from this entry, we discard it, as it is the one internal
node that doesn't split the tree.
#+end_export
...
#+end_src
#+begin_export latex
We iterate over the remaining counts. For each count we calculate the
...
#+end_src
#+begin_export latex
We sort the counts by casting them to the sortable type
\ty{countsSlice}, which we still have to define.
#+end_export
#+begin_src go <<Sort counts, Pr. \ref{pr:fit}>>=
  sort.Sort(countsSlice(counts))
#+end_src
#+begin_export latex
We import \ty{sort}.
#+end_export
...
#+end_src
#+begin_export latex
We print the splits in three columns, clade, parent, and split. We
print the headers for these three columns and then the maximal
splits. These all have the same split percentage as the top scorer.
#+end_export
#+begin_src go <<Print maximal splits, Pr. \ref{pr:fit}>>=
  fmt.Fprint(w, "#Clade\tParent\tSplit (%)\n")
  i := 0
  for i < len(counts) && counts[0].sv == counts[i].sv {
	  fmt.Fprintf(w, "%s\t%s\t%.1f\n", counts[i].label,
		  counts[i].parent, counts[i].sv)
	  i++
  }
#+end_src
#+begin_export latex
