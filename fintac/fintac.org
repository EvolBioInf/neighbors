#+begin_src latex
  ...
  taxonomic targets and neighbors. We further call $v_{\rm t}$, $v_{\rm
    n}$, and $v_{\rm u}$ the number of targets, neighbors, and unknowns
  in the clade rooted on $v$. If the taxonomy always agreed with the
  phylogeny, we could just look for the node that contains all targets
  and no neighbors or unknowns, $v_{\rm t}=n_{\rm t}$ and $v_{\rm
    n}=v_{\rm u}=0$. But if taxonomies and phylogenies always agreed, we
  could restrict our search for targets and neighbors to the output of
  \ty{neighbors}.
  ...
  between targets and neighbors, while also discounting for the presence
  of unknowns. 

  To quantify the contribution of unknowns, we note that positive
  identification---even if wrong---is data, while no identification
  simply isn't. At the same time, when presented with two nodes with the
  same number of targets but differing numbers of unknowns, we prefer
  the node with fewer unknowns. So we discount for the presence of
  unknowns with the logarithm of $v_{\rm u}$ to make it much slower than
  growth due to targets inside and neighbors outside.
  \begin{equation}\label{eq:spl}
    s(v)=\mbox{max}\left\{\begin{array}{l}
	\frac{v_{\rm t} + v^{\ast}_{\rm n}}{n_{\rm n} + n_{\rm t}}\times 100 - \log(v_{\rm u}+1)\\
	0
      \end{array}
  \right .
  \end{equation}
  This quantity ranges from 0, if $v$ contains no targets and all
  neighbors, to 100, if $v$ contains all targets and no neighbors or
  unknowns.
