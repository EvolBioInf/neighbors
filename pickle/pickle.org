#+begin_src latex
  ...
  \begin{figure}
    ...
    \begin{tabular}{cc}
      \textbf{A} & \textbf{B}\\
      \includegraphics{../pickle/tree1.ps} & %% 5cm x 5cm
      \includegraphics{../pickle/tree2.ps}   %% 5cm x 5cm
    \end{tabular}
    \end{center}
    \caption{Full tree for testing the program \ty{pickle} (\textbf{A}),
    and the same tree with clade 8 removed (\textbf{B}).}\label{fig:tree}
  \end{figure}

  The program \ty{pickle} also has a ``complement'' option to get
  everything \emph{but} the nodes in the target clade. When used in tree
  mode, the complement effectively removes the target clade, which gets
  replaced by the number of leaves in its subtree. For example, we can
  remove clade 8 from the tree in Figure~\ref{fig:tree}A, to get
  Figure~\ref{fig:tree}B with $n=5$ where clade 8 used to be.
  \begin{verbatim}
...
#+begin_src go <<Complement clade, Pr. \ref{pr:pic}>>=
  if clade.Parent == nil {
	  ...
	  //<<Remove clade, Pr. \ref{pr:pic}>>
  }
#+end_src
#+begin_export latex
If we print the tree, we'd like to have a visual marker of the removed
clade. So we set the label of the removed clade to the size of the
clade---that is, the number of leaves it contains---and set its child
link to nil. Otherwise, we just remove the clade.
#+end_export
#+begin_src go <<Remove clade, Pr. \ref{pr:pic}>>=
  if *optT {
