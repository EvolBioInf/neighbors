#+begin_src latex
  ...
  requested by the user and prints their leaf labels. Alternatively, it
  prints the requested clade as a Newick tree, which can be rendered
  with any tree drawing program. For example, we can draw clade 9 of the
  test tree to get Figure~\ref{fig:tree}A.
  \begin{verbatim}
  ...
  \begin{figure}
    ...
    and the same tree with node 8 removed (\textbf{B}).}\label{fig:tree}
  \end{figure}

  The program \ty{pickle} also has a ``complement'' option to get
  everything but the nodes in the target clade. In tree mode the
  complement effectively removes the target clade, which gets replaced
  by the number of leaves in its subtree. For example, we can remove
  clade 8 from the tree in Figure~\ref{fig:tree}A, we get
