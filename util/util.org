...
#+end_src
#+begin_export latex
\subsection{\ty{OutlierStatistics}}
  !The function \ty{OutlierStatistics} takes as argument a slice of
  !floats and calculates outlier statistics as defined by the National
  !Institute of Standards and Technology, 
  !https://www.itl.nist.gov/div898/handbook/prc/section1/prc16.htm

  Inside the function, we first create the variable \ty{os} for
  holding the outlier statistics. This variable is of type
  \ty{OutlierStats}, which we still need to define. Then we sort the
  incoming data, determine the median, the lower quartile, the upper
  quartile, the interquartile range, the inner fences, and the outer
  fences.
#+end_export
#+begin_src go <<Functions, Pa. \ref{pa:util}>>=
...
\end{enumerate}
We store these values in the structure \ty{OutlierStats}.
#+end_export
...
#+end_src
#+begin_export latex
We test the function \ty{OutlierStatatistics} on four data points and
compare the results we get to the results we want.
#+end_export
#+begin_src go <<Test \ty{OutlierStatistics}, Pa. \ref{pa:util}>>=
