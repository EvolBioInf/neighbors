#+begin_src latex
  ...
  \ty{names.dmp}, which are part of the dump of the NCBI taxonomy
  ...
  in Table~\ref{tab:nodes}. I have ordered the table of taxon-IDs, but
...
#+begin_src go <<tax.go>>=
  ...
  import (
	  //<<Imports, Pa.~\ref{pa:tax}>>
  )
  //<<Types, Pa.~\ref{pa:tax}>>
  //<<Methods, Pa.~\ref{pa:tax}>>
  //<<Functions, Pa.~\ref{pa:tax}>>
#+end_src
#+begin_src latex
...
#+end_src
#+begin_src go <<Types, Pa.~\ref{pa:tax}>>=
...
#+end_src
#+begin_src go <<Node fields, Pa.~\ref{pa:tax}>>=
...
#+end_src
#+begin_src go <<Functions, Pa.~\ref{pa:tax}>>=
  func NewTaxonomy(nodes, names io.Reader) Taxonomy {
	  tax := make(map[int]*Node)
	  //<<Parse nodes file, Pa.~\ref{pa:tax}>>
	  //<<Construct tree, Pa.~\ref{pa:tax}>>
	  //<<Parse names file, Pa.~\ref{pa:tax}>>
	  return tax
  }
#+end_src
#+begin_src latex
  We import \ty{io}.
#+end_src
#+begin_src go <<Imports, Pa.~\ref{pa:tax}>>=
  "io"
#+end_src
#+begin_src latex
...
#+end_src
#+begin_src go <<Parse nodes file, Pa.~\ref{pa:tax}>>=
  parents := make(map[int]int)
  sc := bufio.NewScanner(nodes)
  for sc.Scan() {
	  row := sc.Text()
	  //<<Split row, Pa.~\ref{pa:tax}>>
	  //<<Store taxon, Pa.~\ref{pa:tax}>>
  }
#+end_src
#+begin_src latex
...
#+end_src
#+begin_src go <<Split row, Pa.~\ref{pa:tax}>>=
...
#+end_src
#+begin_src go <<Store taxon, Pa.~\ref{pa:tax}>>=
  v := new(Node)
  v.ID = taxonID
  tax[taxonID] = v
...
#+end_src
#+begin_src go <<Construct tree, Pa.~\ref{pa:tax}>>=
  for k, v := range parents {
	  if k != v {
		  child := tax[k]
		  parent := tax[v]
		  //<<Set links between parent and child, Pa.~\ref{pa:tax}>>
	  } else {
		tax[k].Parent = tax[k]
	  }
  }
...
#+begin_src latex
  ...
  children.
#+end_src
#+begin_src go <<Set links between parent and child, Pa.~\ref{pa:tax}>>=
  child.Parent = parent
  if parent.Child == nil {
	  parent.Child = child
...
#+end_src
#+begin_src go <<Parse names file, Pa.~\ref{pa:tax}>>=
  ...
  for sc.Scan() {
	  ...
	  //<<Store name, Pa.~\ref{pa:tax}>>
  }
#+end_src
#+begin_src latex
  If the name we've just read is a scientific name, we store it.
#+end_src
#+begin_src go <<Store name, Pa.~\ref{pa:tax}>>=
...
#+begin_src latex
  ...
  \ty{Root}.
#+end_src
#+begin_src go <<Methods, Pa.~\ref{pa:tax}>>=
...
#+begin_src go <<Methods, Pa.~\ref{pa:tax}>>=
  func (t Taxonomy) String() string {
	  str := ""
	  //<<Write tree of taxon-IDs, Pa.~\ref{pa:tax}>>
	  //<<Write table of names, Pa.~\ref{pa:tax}>>
	  return str
  }
#+end_src
#+begin_src latex
...
#+end_src
#+begin_src go <<Write tree of taxon-IDs, Pa.~\ref{pa:tax}>>=
  var b []byte
...
#+end_src
#+begin_src go <<Write table of names, Pa.~\ref{pa:tax}>>=
...
#+end_src
#+begin_src latex
  To see an example of how this counter is used, consider again the taxa
  4 and 7 in Figure~\ref{fig:tax}. We set their descendant counters to 1
  and climb to nodes 3 and 5. We increment their descendant counters to
  1, and since none of the counters is equal to 2, we climb to nodes 1
  and 3. Now the counter of 3 is incremented to 2 and we have found the
  MRCA.
...
#+begin_src go <<Methods, Pa.~\ref{pa:tax}>>=
  ...
  func (t Taxonomy) MRCA(taxa []int) int {
	  var mrca *Node
	  //<<Convert taxon-IDs to children, Pa.~\ref{pa:tax}>>
	  //<<Search for MRCA, Pa.~\ref{pa:tax}>>
	  //<<Reset descendant counters, Pa.~\ref{pa:tax}>>
	  return mrca.ID
  }
#+end_src
#+begin_src latex
...
#+end_src
#+begin_src go <<Convert taxon-IDs to children, Pa.~\ref{pa:tax}>>=
  var children []*Node
...
#+end_src
#+begin_src go <<Search for MRCA, Pa.~\ref{pa:tax}>>=
  for mrca == nil {
	  //<<Increment descendant counter, Pa.~\ref{pa:tax}>>
	  //<<Climb tree, Pa.~\ref{pa:tax}>>
  }
#+end_src
#+begin_src latex
...
#+end_src
#+begin_src go <<Increment descendant counter, Pa.~\ref{pa:tax}>>=
...
#+end_src
#+begin_src go <<Climb tree, Pa.~\ref{pa:tax}>>=
  var parents []*Node
  for _, child := range children {
	  if child.ID > 1 {
...
#+end_src
#+begin_src go <<Reset descendant counters, Pa.~\ref{pa:tax}>>=
  resetNumDesc(mrca)
#+end_src
#+begin_src latex
...
#+begin_src go <<tax_test.go>>=
  ...
  import (
	  "testing"
	  //<<Testing imports, Pa.~\ref{pa:tax}>>
  )
  //<<Testing functions, Pa.~\ref{pa:tax}>>
#+end_src
#+begin_src latex
...
#+end_src
#+begin_src go <<Testing functions, Pa.~\ref{pa:tax}>>=
  func TestMethods(t *testing.T) {
	  //<<Read taxonomy, Pa.~\ref{pa:tax}>>
	  //<<Test \ty{String}, Pa.~\ref{pa:tax}>>
	  //<<Test \ty{MRCA}, Pa.~\ref{pa:tax}>>
	  //<<Test \ty{Leaves}, Pa.~\ref{pa:tax}>>
	  //<<Test \ty{Subtree}, Pa.~\ref{pa:tax}>>
  }
#+end_src
#+begin_src latex
...
#+end_src
#+begin_src go <<Read taxonomy, Pa.~\ref{pa:tax}>>=
...
#+end_src
#+begin_src go <<Testing imports, Pa.~\ref{pa:tax}>>=
  "os"
  "fmt"
#+end_src
#+begin_src latex
  To test \ty{String}, we sort the strings we get so our result is
  reproducible.
#+end_src
#+begin_src go <<Test \ty{String}, Pa.~\ref{pa:tax}>>=
  want, err := ioutil.ReadFile("r.txt")
  if err != nil { t.Error(err) }
  gstr := tax.String()
  gsli := strings.Split(gstr, "\n")
  sort.Strings(gsli)
  get := []byte(strings.Join(gsli, "\n"))
  if !bytes.Equal(get, want) {
	  t.Errorf("get:\n%s\nwant:\n%s\n", get, want)
  }
#+end_src
#+begin_src latex
  We import \ty{ioutil}, \ty{sort}, \ty{strings}, and \ty{bytes}.
#+end_src
#+begin_src go <<Testing imports, Pa.~\ref{pa:tax}>>=
  "io/ioutil"
  "sort"
...
#+end_src
#+begin_src go <<Test \ty{MRCA}, Pa.~\ref{pa:tax}>>=
  //<<Construct tests of \ty{MRCA}, Pa.~\ref{pa:tax}>>
  //<<Run tests of \ty{MRCA}, Pa.~\ref{pa:tax}>>
#+end_src
#+begin_src latex
  ...
  \end{itemize}
#+end_src
#+begin_src go <<Construct tests of \ty{MRCA}, Pa.~\ref{pa:tax}>>=
  taxa := make([][]int, 0)
  var res []int
...
#+end_src
#+begin_src go <<Run tests of \ty{MRCA}, Pa.~\ref{pa:tax}>>=
...
#+begin_src latex
  ...
  4, 7, and 6.
#+end_src
#+begin_src go <<Test \ty{Leaves}, Pa.~\ref{pa:tax}>>=
  leaves := tax.Leaves(3)
  s := 0
...
#+end_src
#+begin_src go <<Test \ty{Subtree}, Pa.~\ref{pa:tax}>>=
  sub := tax.Subtree(3)
  s = 0
  for _, t := range sub {
	  s += t
