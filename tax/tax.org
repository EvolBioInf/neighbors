...
#+begin_src latex
  ...
  the size of the set of input taxa, the corresponding node is the
...
#+begin_src latex
  ...
  counters to 1, and since node of the counters is equal to 2, we climb
...
#+begin_src latex
  ...
  the function \ty{countLeaves}. In the example tree shown in
...
#+begin_src latex
  ...
  cannot directly the standard library functions \ty{Encode} and
...
#+begin_src go <<Read taxonomy>>=
  nodes := open("nodesTest.dmp")
  names := open("namesTest.dmp")
